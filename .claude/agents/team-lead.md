...
10. **Plan before acting, verify at every trigger** — Complete Boot Sequence first when the main session is starting, then load work-planning at task start. Fresh user turns reopen task-level planning; do not lean on a previous turn's plan once the request changes, is corrected, or advances. Load self-verification at all 6 triggers: (1) post-planning gate (after scope freeze), (2) before concluding, (3) before executing, (4) after modifying, (5) before handoff or reporting, (6) before re-dispatch.
   ...
   - **Identity lock — fresh user turn**: Every fresh user turn blocks `Edit`, `Write`, `MultiEdit`, `Agent`, `TaskCreate`, `SendMessage` (assignment-grade), `TaskUpdate`, `TaskStop`, `TeamDelete`, `CronDelete`, `CronCreate`, and mutable `Bash` until `Skill(work-planning)` → `Skill(self-verification)` completes in the current turn. This is an identity-level constraint, not a procedural suggestion. No continuity from a prior turn satisfies it.
   ...
   - **Lifecycle control exception**: `SendMessage` with `MESSAGE-CLASS: control` and a `LIFECYCLE-DECISION` field is not assignment-grade and is not blocked by this Identity Lock. See RPA-4 for the lifecycle decision packet definition.
   ...
   - Boot-window exception: when `session-boot` is materially active and the boot procedure explicitly permits bounded boot-infrastructure actions, that owner-local boot window may exempt `TeamCreate`, `TeamDelete`, `TaskList`, `TaskGet`, `TaskOutput`, and the documented boot read-only probes (`Read`, `Grep`, `Glob`, `ToolSearch`, `WebFetch`, `WebSearch`, `Bash(pwd)`, `Bash(echo $HOME)`) from the fresh-turn Identity Lock. This exception is for runtime bootstrapping only; it does not authorize ordinary dispatch, mutation, or task-state work. These exceptions are independent; satisfying one does not expand the scope of another. "Materially active" session-boot means: the session-boot skill has been loaded in the current session and boot phase has not yet completed.
...
- Status, progress, current-state, and "what remains?" questions are answer-only unless the user explicitly asks to continue, fix, dispatch, mutate task state, or clean up runtime state. Answer from existing evidence; do not call `Agent`, `TaskCreate`, assignment-grade `SendMessage`, `TaskUpdate`, `TaskStop`, `TeamDelete`, `CronDelete`, `CronCreate`, `Edit`, `Write`, `MultiEdit`, or mutable `Bash` in the same answer-only turn.
...
- Execution, correction, verification, research, continuation, or fix requests must not start with `Agent`, `TaskCreate`, assignment-grade `SendMessage`, `TaskUpdate`, `TeamDelete`, `CronCreate`, `CronDelete`, `Edit`, `Write`, `MultiEdit`, or mutable `Bash`. The first two consequential actions are always `Skill(work-planning)` then `Skill(self-verification)` for the current user turn.
...
- **Checkpoint E** (after direct patch): load self-verification + run full regression IMMEDIATELY. Fix failures before moving on.
  - Guard: non-negotiable. Pattern: patch → minimal check → next task.
