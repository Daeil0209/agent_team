...
10. **Plan before acting, verify at every trigger** — Complete Boot Sequence first when the main session is starting, then load work-planning at task start. Fresh user turns reopen task-level planning; do not lean on a previous turn's plan once the request changes, is corrected, or advances. Load self-verification at these gates: (1) post-planning (after scope freeze), (2) before concluding, reporting, or handoff, (3) before executing or re-dispatching, (4) after modifying. Co-occurrence rules in `skills/self-verification/SKILL.md §Activation Trigger` govern when a single load satisfies multiple gates.
   ...
   - **Request-fit outcome lens:** When a request includes a concrete deliverable, supplied reference, burden-reduction cue, or correction for over-asking, `work-planning` applies `request-intent-fit` as an interpretation lens before surfacing questions. Success is measured by the planned outcome: concrete user-visible deliverable, reference-backed defaults, and only material blockers surfaced. Use an explicit `Skill(request-intent-fit)` load when that inference is non-trivial or prior behavior showed over-asking; trivial fully specified work can apply the lens mentally.
...
- Routine status, progress, current-state, and "what remains?" questions are answer-only unless the user explicitly asks to continue, fix, dispatch, mutate task state, or clean up runtime state. Runtime recovery exceptions below are not routine status. Answer routine status from existing evidence; do not call `Agent`, `TaskCreate`, `Edit`, `Update`, `Write`, `MultiEdit`, or mutable `Bash` in the same answer-only turn.
- Exception: if existing evidence shows `unclaimed-dispatch-failure`, `idle-no-completion`, or another active runtime handoff failure, classify the turn as correction rather than routine status. Answer briefly, then select the recovery path from the dispatch reception channel without asking the user to choose.
...
- Execution, correction, verification, research, continuation, or fix requests must not start with `Agent`, `TaskCreate`, `Edit`, `Update`, `Write`, `MultiEdit`, or mutable `Bash`. The first two consequential actions are always `Skill(work-planning)` then `Skill(self-verification)` for the current user turn.
...
- Scope rule: keep `work-planning` and `self-verification` on consequential paths. Read-only bootstrap inspection and bounded task lookup (`TaskList`/`TaskGet`/`TaskOutput`) may gather context without reloading those skills unless the next move crosses into dispatch, semantic mutation, execution, or completion reporting. Bounded `TaskUpdate`/`TaskStop` may record already-established runtime truth when the exact task id is known; it does not consume workflow cursor, prove completion, or authorize new work, and a half-planned current execution still requires post-planning self-verification first.
