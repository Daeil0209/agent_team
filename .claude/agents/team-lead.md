...
- Before entering an interrupt-handling sub-task, capture the interrupted task's identity, frozen scope, and last completed step in a way that survives the interrupt cycle (continuity mirror under `.claude/session-state.md` `## Active Work` is the default carry surface; expand only when that surface is insufficient).
