...
standby_worker_count_for_surface() {
  local target_surface="${1-}"
  local norm_surface=""

  [[ -f "$STANDBY_FILE" ]] || { printf '0'; return 0; }

  norm_surface="$(printf '%s' "$target_surface" | tr '[:upper:]' '[:lower:]' | sed 's/^[[:space:]]*//; s/[[:space:]]*$//')"

  awk -v norm_surf="$norm_surface" -v log_dir="$LOG_DIR" '
    function trim(value) {
...
standby_worker_summary_for_surface() {
  local target_surface="${1-}"
  local norm_surface=""

  [[ -f "$STANDBY_FILE" ]] || return 0
  ...
  awk -v norm_surf="$norm_surface" -v log_dir="$LOG_DIR" '
    ...
    {
      worker = trim($0)
      worker_key = tolower(worker)
      if (worker == "" || seen[worker_key]) {
        next
      }
      seen[worker_key] = 1
      if (norm_surf != "") {
        wsurface = worker_surface(worker_key)
        if (wsurface != "" && wsurface != norm_surf) {
          next
        }
      }
