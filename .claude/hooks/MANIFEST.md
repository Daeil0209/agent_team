...
| `pre-consequential-action-skill-verification-gate.sh` | RC-B self-rule blind spot enforcement at runtime composition moment (essence-discipline F-S4 curtain-self-violation pattern closure) + RC-D role-separation gap enforcement at SendMessage assignment-grade moment (F-S2 premature-dispatch closure) + RC-I lead-internal-aid boundary enforcement at Bash test-pattern moment (F-S3 self-test architectural-enablement closure) + RC-G surface-pattern-matching enforcement at .claude/* mutation moment (F-S1 surface-trigger-proposal closure, currently advisory pending sv-tracker integration). Approval basis: team-meeting `team-lead-essence-discipline` r2 4/4 critic CONSENT (developer T14+T18 / researcher T15+T19 / reviewer-shard-a T16 / reviewer-shard-b T17) + per-S 16-row INVARIANT matrix + Hook-Last review (this ledger row). Carrier: `claude_doc/governance-audit-2026-05-27/meetings/team-lead-essence-discipline/draft-r2.md`. | PreToolUse on Edit/MultiEdit/Write/SendMessage/Bash with 5-second timeout; fail-open posture on hook-internal error. Branch 1 (governance mutation on `.claude/*`) advisory-only at install — direct PreToolUse transcript scan infeasible (no transcript access); named follow-on path = sv-tracker.sh-style PostToolUse Skill load state file + this hook reading that state file. Branch 2 (assignment-grade SendMessage `MESSAGE-CLASS: assignment\|reuse\|reroute` first-line regex match) active-deny on missing `DISPATCH-AUTHORIZATION-BASIS:` field per `.claude/skills/task-execution/references/assignment-packet.md` floor-field rule; recover by adding the field with named basis token. Branch 3 (Bash test/proof/validation pattern detection) active-deny on TEST_PATTERN_DIRECT (pytest/jest/playwright/vitest/cypress/mocha/wget/nodemon/rspec/phpunit) / TEST_PATTERN_RUNNER (npm/pnpm/yarn/go/cargo `test`) / TEST_PATTERN_CURL (curl with `-X POST/PUT/DELETE/PATCH` or `--request POST/...`); carve-outs (codex CLI per Pattern A / `--version` / `--help` / `-V` / `-h` / non-test commands default-allow); recover by routing to tester/validator lane via assignment with `DISPATCH-AUTHORIZATION-BASIS: frozen-independent-lane:<lane>`. |
...
- INPUT_JSON Consumer Inventory (post-Wave-4 Batch H closure per W4-CX-2 refinement):
  - **Fully migrated to `parseInput()` (10 hooks)**: `agent-activity-monitor.sh`, `compliance-supervisor.sh` (lines :6 short session_id + :1115 heredoc), `permission-request-gate.sh`, `session-end.sh`, `session-start.sh`, `sv-gate.sh`, `sv-tracker.sh`, `track-runtime-lifecycle.sh` (lines :49 short tool_name + :59 Agent heredoc + :180 TeamCreate/TeamDelete heredoc), `track-worker-transport.sh` (lines :29 + :90), `user-prompt-gate.sh`, `worker-lifecycle-sync.sh` (lines :10 + :230).
  - **Not migration-feasible — different parse pattern (shell stdin-grep, no node JSON.parse)**: `assistant-response-curtain-gate.sh` (uses `grep -oE '"text"...'` shell extraction at Stop event), `reporting-curtain-envelope-gate.sh` (uses `grep -qE`/`grep -oE` shell pattern matching on PreToolUse SendMessage), `pre-consequential-action-skill-verification-gate.sh` (uses `grep -oE` shell extraction for first-line `MESSAGE-CLASS:` regex match). These hooks parse stdin via shell pattern matching for narrow-line extraction, not node JSON.parse — the parseInput helper's node-side interface does not apply. Recorded as `rejected:non-applicable:shell-stdin-grep-pattern-mismatch` per Wave 4 Batch H remaining-consumer migration closure.
  - **Other lib utilities (do not consume INPUT_JSON)**: `health-check.sh` (reads from CONFIG_FILE not INPUT_JSON), `tmux-kill-block.sh` (pattern-deny only). Not in P6.2 consumer scope.
