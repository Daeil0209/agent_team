...
| `sv-tracker.sh` | Skill load host-result display becoming visible even though skill-load/learned narration is non-reportable | PostToolUse Skill only; records session skill markers and emits `suppressOutput` for recognized Skill invocations, accepting both `tool_input.skill` and `tool_input.name` input shapes. Model-side skill content remains available; only user-visible tool-result display is suppressed. Recover by keeping skill-load evidence in hook state or governed carriers, not visible prose. |
