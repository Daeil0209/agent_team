...
| `task-start-gate.sh` | consequential work starting without required procedure/runtime state | Broad core/Web matcher plus narrow Codex/Playwright MCP edges; agent work before assignment `dispatch-ack` is blocked; other preflight gaps warn unless exact delivery/runtime damage is imminent. Recover by sending receipt, opening the missing owner, closeout owner, or narrowing result truth. |
