...
| `task-start-gate.sh` | consequential work starting without required procedure/runtime state | Broad core-tool matcher plus narrow Codex/Playwright MCP edges; agent work before assignment `dispatch-ack` is blocked; other preflight gaps warn unless exact delivery/runtime damage is imminent. Recover by sending receipt, opening the missing owner, closeout owner, or narrowing result truth. |
| `sv-gate.sh` | browser proof without current planning basis | Narrow Playwright proof edge only; warning is preferred when denial would prevent ledger repair; recover through work-planning or claim narrowing. |
...
| `dispatch-sizing-gate.sh` | exact duplicate live target, oversized packet, mixed-purpose agent packet, or hidden merge risk | `Agent` only; hard-deny live target collision. Standby, idle-pending, and recoverable packet gaps warn, then route through packet repair, lifecycle/reuse recovery, or lane `hold\|blocker`. |
...
| `compliance-supervisor.sh` | `.claude` governance shell mutation, protected reference mutation, destructive recursive cleanup, disallowed output roots, hook-runtime artifacts, or interpreter-based filesystem bypass | Mutation-capable tools only; recursive delete remains hard-denied unless current user intent arms one exact workspace child root, including the active team runtime project path recorded in config description; recover through structured edit, user-authorized recursive cleanup, or repo-root project output paths. |
