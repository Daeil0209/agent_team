...
- Procedure gaps, planning/self-verification order gaps, packet-quality gaps, review-tool absence, output-root instructions, SendMessage content-class quality, and recoverable completion defects are not hard-deny causes unless they directly create one of those reserved dangers.
...
- The `SendMessage` string-message summary guard is a backstop for the current Claude Code schema failure where string `message` requires `summary`; accepted but doctrine-malformed state-signal envelopes route to the Communication Plane owner and post-use transport tracker.
...
- Hard-deny existing governance-file wholesale overwrite, secret/credential mutation or explicit Bash read, runtime/team-state overwrite, team runtime directory removal, shell mutation whose destination or effective mutation target is `.claude`, protected-filesystem bypass by non-developer worker `Write`/`Edit`/`MultiEdit` outside its frozen `RETAINED-OUTPUT-PATH` or declared `WRITE-SCOPE`, interpreter-based protected-path bypass, mutable protected/outside-workspace `rm` or `find`, catastrophic primitives, and `SendMessage` string `message` without required `summary`.
- Display-only shell stdout is not a compliance-supervisor hard-deny category; apply `.claude/reference/work-runtime-boundary-law.md` `### Bash Internal Evidence Capture Contract` instead.
