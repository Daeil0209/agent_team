...
- Use `permissionDecision: "deny"` only for reserved hard-deny dangers, exact runtime actor/tool impossibilities that would corrupt delivery or runtime truth, or the observed Claude Code `SendMessage` schema failure where string `message` lacks required `summary`.
...
- Assistant-authored Bash `echo`/`printf` progress, start, completion, or banner text remains governed by doctrine and pre-action rules, not hook hard-deny; hooks must not block it unless the command also triggers a reserved hard-deny danger.
- Do not add a new hook when adherence to an existing doctrine, skill, or role
surface is the real fix.
- The `SendMessage` string-message summary guard is a backstop for the current Claude Code schema failure where string `message` requires `summary`; accepted but doctrine-malformed state-signal envelopes route to the Communication Plane owner or post-use transport tracker, not PreToolUse hard-deny.
...
| `compliance-supervisor.sh` | `.claude` governance mutation, wholesale overwrite, catastrophic primitives, runtime/team-state corruption, secrets exposure, hook-runtime artifact misplacement, non-developer own-output scope violation, protected-filesystem bypass, or `SendMessage` string `message` without required `summary` | Mutation-capable tools plus explicit Bash secret-read commands and PreToolUse `SendMessage` prevention for the deterministic Claude Code schema failure where string `message` lacks `summary`. Recover by using non-secret evidence, structured edit paths, bounded filesystem targets, adding `summary`, using a structured message object, or `summary: dispatch-ack/subjob-done` with `message: " "`. |
...
- Hard-deny existing governance-file wholesale overwrite, secret/credential mutation or explicit Bash read, runtime/team-state overwrite, team runtime directory removal, shell mutation whose destination or effective mutation target is `.claude`, protected-filesystem bypass by non-developer worker `Write`/`Edit`/`MultiEdit` outside its frozen `RETAINED-OUTPUT-PATH` or declared `WRITE-SCOPE`, interpreter-based protected-path bypass, protected/outside-workspace `rm` or `find`, catastrophic primitives, and `SendMessage` string `message` without required `summary`.
- Bash progress/banner stdout is not a compliance-supervisor hard-deny category; repair unnecessary Bash labels through doctrine, pre-action discipline, and report-curtain rules.
