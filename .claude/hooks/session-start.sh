...
# (5) Verify constitutional curtain hooks wired in settings.json
...
if ! grep -q 'sv-tracker.sh' "$CLAUDE_ROOT/settings.json" 2>/dev/null; then
  curtain_issues+=("settings.json missing sv-tracker.sh wiring (Skill-result suppression curtain layer compromised)")
fi

# (6) Verify constitutional curtain hook scripts exist with non-empty body
for curtain_hook in reporting-curtain-envelope-gate.sh sv-tracker.sh; do
...
# (8) Verify PROTECTED-CURTAIN-SURFACE markers on key curtain text surfaces
...
# (9) Verify canonical PROTECTED-CURTAIN-SURFACE enumeration wording-drift detection
...
if [[ -f "$canonical_enum_check" ]]; then
  for expected_token in 'Constitutional Reporting Curtain' 'reporting-core-law.md' 'reporting-prohibition-law.md' 'Constitutional Curtain Protection' 'curtain-breach' 'Curtained Communication' 'reporting-curtain-envelope-gate.sh' 'sv-tracker.sh' 'curtain-validation block' 'curtain hook wiring'; do
