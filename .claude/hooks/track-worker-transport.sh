...
CONVERGENCE_PASS_STATE="absent"
LANE_LOCAL_RESULT_VERIFICATION_STATE="absent"
...
if [[ -n "$CONVERGENCE_PASS_VALUE" ]]; then
  CONVERGENCE_PASS="true"
  CONVERGENCE_PASS_STATE="present-unverified"
fi
if [[ -n "$(printf '%s' "$LANE_LOCAL_RESULT_VERIFICATION_VALUE" | tr -d '[:space:]')" ]]; then
  LANE_LOCAL_RESULT_VERIFICATION="true"
  LANE_LOCAL_RESULT_VERIFICATION_STATE="present-unverified"
fi
...
const line = {
  ...
  fields: {
    ...
    convergencePassFieldPresent: process.env.TRANSPORT_CONVERGENCE_PASS === "true",
    laneLocalResultVerificationFieldPresent: process.env.TRANSPORT_LANE_LOCAL_RESULT_VERIFICATION === "true"
  },
  fieldTruth: {
    convergencePass: process.env.TRANSPORT_CONVERGENCE_PASS_STATE || "absent",
    laneLocalResultVerification: process.env.TRANSPORT_LANE_LOCAL_RESULT_VERIFICATION_STATE || "absent"
  },
  fieldValues: {
