...
Stop before closure when a changed top-doctrine execution rule lacks mapped core-law executable detail or explicit existing-detail coverage.
Stop before closure when changed core-law executable detail lacks required trigger-bound owner guidance or explicit not-material basis.
Stop before mutation when the patch target is not consumed by the failing execution path.
Stop before mutation when an added or changed reference path lacks direct-consumption relevance.
