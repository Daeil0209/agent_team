...
4. Required `SV-RESULT` is current for analyzed, evaluated, synthesized, produced, strengthened, phase/stage-end, or completion-result claims.
...
Do not emit tool-adjacent prose for planning freeze, reference loading, Codex or reviewer adjudication, packet drafting, directory creation, task updates, dispatch preparation, monitoring, or recovery while Procedure Plane or Communication Plane can continue.
Examples of suppressed tool-adjacent prose include `Boot closes`, `Corpus measured`, `Sharding plan`, `Internal plan freeze`, `Required mode`, `Internal planning record is frozen`, `Codex returned`, `adjudicating now`, `planning freeze complete`, `opening task-execution`, `I'll set up`, `dispatching now`, `loading`, `starting`, `will report`, `findings to`, `shard arrived`, `audit complete`, `4/7 complete`, and per-shard findings summaries.
If the user explicitly asks why an expected action did not happen, answer only with the violated rule/owner, the direct procedural cause, and the current correction owner/action; do not include self-accusation, excuses, intent promises, proceed prompts, or internal packet burden narrative.
...
`explicit status answer` answers only the user's question and omits internal topology, packet, shard, and evidence detail unless specifically requested.
