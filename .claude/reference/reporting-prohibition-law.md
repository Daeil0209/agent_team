...
- Duplicate assistant-authored rendering of the same internal state token across visible envelope slots is a report attempt; the Communication Plane owner must render at most one no-detail token and keep receiver-required detail in internal carriers.
