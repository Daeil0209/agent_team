...
## Consume When
- Consume before any assistant-authored visible prose, terminal-visible report text, pane-visible report text, visible task-plan/todo/checklist text, completion claim, phase/stage-end result, blocker report, explicit status answer, or closeout residual.
...
4. `final verified result` requires all same-request executable owner/actions closed, explicitly cancelled/redirected, lawfully owner-deferred/out-of-scope, or blocked by truthful `HOLD`.
...
6. Multi-lane, multi-shard, review/proof/validation, or validator-routed work requires every required result to be completion-grade, retained-carrier consumed, integrated, and accepted, lawfully deferred/out-of-scope, or blocker-classified before `final verified result`.
...
## Report Shape
- `final verified result` states the final result first, in concise Korean, with only essential verification basis, material open surface, blocker truth, or canonical artifact path.
