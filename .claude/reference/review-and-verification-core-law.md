...
- `renderable-projection-violation` requires terminal stdout/stderr or another exact display-output edge that would expose non-reportable content while the receiver-required information and owner action can continue through quiet retry or a governed carrier.
...
- Hook eligibility is valid only when the failure source is `reserved-runtime-danger`, proven `tool-schema-failure`, or a display-output `renderable-projection-violation`; the proposed guard must block only the rejected, reserved-danger, or display-output shape, preserve internal information flow, and fail the stronger narrower alternative test.
