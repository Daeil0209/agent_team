...
- Narrowing-intent must route through explicit `scope-pressure` to the dispatching owner (or caller for skill-internal cases) with: (i) the dispatched/caller-supplied/frozen scope, (ii) the proposed narrower scope, (iii) the rationale for narrowing, (iv) the explicit dispatcher/caller-confirm request; the owner waits for confirm reply before proceeding under the narrower scope.
