...
- Candidate discovery maximizes recall for evidence collection while preserving claim ceiling, then filters by eliminating defeated alternatives: apparent defects also record visible target-intent, protected-function, direct-consumption, non-executable-difference, scope, stale-basis, or rejection-basis evidence that could defeat promotion.
...
- Lane-local conclusion is the assigned lane's elimination-first classification after rejecting unsupported, unowned, out-of-scope, stale, protected-restatement, non-executable-difference, or unverified alternatives inside that lane surface; it reduces the next owner's review burden but stays inside the lane's `CLAIM-CEILING` until current `Skill(review-verification)` promotion basis or lawful team-lead synthesis consumes it.
...
- Missing any `confirmed-defect` element keeps the item at `candidate-evidence`, `candidate-classified`, `open-candidate`, or `rejected:<basis>`; reviewer agreement, repeated wording, label similarity, apparent-defect framing, plausible future risk, explanation fluency, ordering pressure, or repair-order need cannot substitute for any element.
...
- Raw candidate counts, repeated labels, similar wording, multi-lane convergence, and lane-local conclusions remain bounded evidence unless this ladder promotes them beyond their `CLAIM-CEILING`.
...
- Ordinary governed `problem-report`, `dispatch-ack`, `scope-pressure`, `hold|blocker`, and accepted completion-grade `subjob-done` retained-carrier handoff are protocol promises, not cause/remedy cases; classify only a defect, malformed transport, no-patch, hook/runtime/schema, or reporting-remedy judgment about those signals.
...
- `failure-source` is exactly one of: `governance-rule-defect`, `tool-schema-failure`, `reserved-runtime-danger`, `renderable-projection-violation`, `screen-pollution`, `procedure-bottleneck`, `evidence-gap`, `non-issue`, or `mixed:<split-required>`.
...
- `hook-eligibility` is exactly `eligible:<tool-schema-failure|reserved-runtime-danger|renderable-projection-violation>:<evidence>` or `not-eligible:<basis>`.
...
- `renderable-projection-violation` requires a controllable assistant-authored renderable field or proven hook-supported projection edge that would expose non-reportable assistant-authored content while receiver-required information and owner action can continue through a governed carrier or corrected owner path.
- `screen-pollution`, reporting-law violation, communication-plane malformed display, SendMessage content-class quality, procedure bottleneck, governance wording defect, or evidence gap is not hook-eligible on that basis alone.
- Hook eligibility is valid only when the failure source is `reserved-runtime-danger`, proven `tool-schema-failure`, or a hook-supported `renderable-projection-violation`; the proposed guard must block only the rejected, reserved-danger, or proven projection shape, preserve internal information flow, and fail the stronger narrower alternative test.
