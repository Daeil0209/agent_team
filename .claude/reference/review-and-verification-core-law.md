...
- Ladder ceiling scope: the `candidate-classified` upper bound for reviewer-lane / non-lead-synthesis work applies to findings AGAINST the audited corpus or assigned target surface. Findings ABOUT the reviewer's or lane agent's own packet's internal integrity (citation completeness, classification correctness within the evidence-only ceiling, field-format discipline, packet structural defects) flow through `Skill(self-verification)` Step 5 Recorrection cycle, which has no finding-state-ladder ceiling restriction: the producer self-corrects own carrier defects via re-Write or `Edit`/`MultiEdit` within frozen `WRITE-SCOPE` and reruns Steps 2-3 until convergence. The two scopes are mutually exclusive; conflating them either suppresses legitimate own-packet self-correction (treating an own-packet defect as ceiling-bound) or unlawfully escalates own-packet findings into corpus-level promotion (treating own-packet correction as substantive corpus-promotion authority).
