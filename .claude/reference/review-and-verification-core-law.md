...
- When the frozen claim or deliverable opens promotion filtering, each opened candidate remains open until it is promoted, rejected by current `Skill(review-verification)` basis, covered by cited lawful owner-deferral authority, excluded by explicit user-narrowed out-of-scope basis, or blocked by truthful `HOLD`.
...
- A governance audit deliverable whose frozen scope includes defect promotion stays open while any opened candidate lacks promotion, current-review rejection, cited lawful owner-deferral authority, explicit user-narrowed out-of-scope basis, or truthful `HOLD`.
- A governance audit-plus-patch deliverable also stays open while any confirmed defect lacks patch-worthiness judgment, any patch-worthy defect lacks patch-ready basis, or any patch-ready row remains unapplied, unverified, deferred by cited lawful owner-deferral authority, rejected by current review basis, or blocked by truthful `HOLD`.
...
- Governance-related `candidate-classified` requires a recorded candidate-classification basis gate: `TARGET-INTENT-BASIS`, suspected negative operating effect, causal-path hypothesis, and tested rejection attempt for `protected-restatement`, `design-tradeoff`, `non-issue`, and `not-material:<basis>`.
- Missing recorded candidate-classification basis keeps the governance-related item at `candidate-evidence` or `open-candidate`; it must not be counted, routed, or worded as a classified governance defect.
...
- Ladder ceiling scope (corpus findings): the `candidate-classified` upper bound for reviewer-lane / non-lead-synthesis work applies to findings AGAINST the audited corpus or assigned target surface.
- Own-packet integrity scope: findings ABOUT the reviewer's or lane agent's own packet's internal integrity (citation completeness, classification correctness within the evidence-only ceiling, field-format discipline, packet structural defects) flow through `Skill(self-verification)` Step 5 Recorrection cycle.
- Own-packet self-correction authority: `Skill(self-verification)` Step 5 Recorrection cycle has no finding-state-ladder ceiling restriction; the producer self-corrects own carrier defects via re-Write or `Edit`/`MultiEdit` within frozen `WRITE-SCOPE` and reruns Steps 2-3 until convergence.
- Scope exclusivity: the corpus-finding scope and the own-packet-integrity scope are mutually exclusive; conflating them either suppresses legitimate own-packet self-correction (treating an own-packet defect as ceiling-bound) or unlawfully escalates own-packet findings into corpus-level promotion (treating own-packet correction as substantive corpus-promotion authority).
...
- Repeated promotion of non-defects is judged on the promoting packet, synthesis, or review method rather than on the audited corpus.
- `promoter-calibration-defect` exists when the same promotion owner raises three or more governance-related `candidate-classified` or stronger items without the recorded candidate-classification basis gate in one packet, synthesis, or same-request chain.
- `promoter-calibration-defect` also exists when the same promotion owner repeats the same non-defect promotion logic three or more times after current rejection basis exists, or reuses a rejected basis as defect evidence without new material evidence.
- A `promoter-calibration-defect` routes to the owner of the promoting packet, synthesis, or review method and does not convert the audited corpus item into a defect.
...
## Anti-Narrowing Law
- Review and verification owners (`validator`, `Skill(review-verification)` callers, `Skill(self-verification)` callers, reviewer-lane) must NOT silently narrow review/verification scope below the dispatched, caller-supplied, or frozen scope basis.
- Narrowing-intent must route through explicit `scope-pressure` to the dispatching owner (or caller for skill-internal cases) with: (i) the dispatched/caller-supplied/frozen scope, (ii) the proposed narrower scope, (iii) the rationale for narrowing, (iv) the exact confirmation needed from that owner. The owner continues under the original full scope by default when safe; narrower-scope execution starts only after owner confirmation, and `HOLD` records why full-scope continuation is impossible when safe full-scope work cannot continue.
- Named negative cases requiring explicit confirm: excluding sub-elements of the assigned acceptance/review surface, deferring an in-scope acceptance/review row to a "separate path" without explicit dispatcher direction, sampling/wave/priority-tier narrowing without dispatcher-frozen sample/wave/tier basis, scoping-out a finding-class without dispatcher direction.
- Silent narrowing is procedural failure regardless of whether the narrower scope's substantive conclusion would otherwise be correct on the narrower scope; substantive-correctness on a silently-narrowed surface does not cure the procedural-adherence defect.
- Confirmed silent narrowing without dispatcher confirm is a procedure-adherence defect and an execution-force defect, classified as `confirmed-defect:silent-narrowing` and blocking positive review/verification/acceptance claims on the affected surface until corrected (re-verification on full dispatched/caller-supplied/frozen scope, or dispatcher-confirmed narrower scope).
- This law is owner-boundary-adjacent (see `## Owner Boundary And Skill Consumption Law`): owner-boundary covers cross-lane movement; anti-narrowing covers within-owner scope discipline; both are positive routing/scope-coverage requirements for R&V owners.
...
- Per `.claude/CLAUDE.md` `## 5. Modification Philosophy` comprehension-as-execution-force basis, misunderstanding-inducing wording that causes proven executable-action impedance is a continuous execution-impedance defect, subordinate to `removal-first` and `minimum-executable-information` discipline.
- Defect-candidate promotion for comprehension-friction requires two conjunctive elements: a textual-feature class match from the catalogue below AND a trace to proven executable-action impedance from the impedance-class catalogue below.
- Textual-feature classes causing comprehension misalignment:
  - vocabulary overlap masking different operational meaning
  - closed-list "only" framing that hides exception salience
  - identifier ambiguity without scope qualifier
  - structural tension between criterion and enumeration
  - other comprehension-friction features at the same operational level
- Proven executable-action impedance classes:
  - wrong owner selection
  - wrong next action
  - stale information consumption
  - failed boundary verification
- A defect-candidate promotion is sufficient evidence of comprehension-friction defect at that surface only when both a textual-feature class match AND a traced impedance-class match are recorded for the same defect candidate. Defect-candidate promotion by any reviewer (including self in audit, review, or re-reading) lawfully applies this rule.
...
- PROTECTED-CURTAIN-SURFACE (curtain-breach sub-rule): the immediately-following `curtain-breach` defect-class definition is enumerated in `.claude/reference/modification-core-law.md` `## Constitutional Curtain Protection`. Removal-first / consolidate / tighten / reduce / sweep operations on this sub-rule require validator pre-approval + user notification.
- `curtain-breach` is a named confirmed-defect class governed by `.claude/CLAUDE.md` `## Constitutional Reporting Curtain` and `.claude/reference/reporting-prohibition-law.md` `## Pre-Report Gate`. Promotion criteria: any assistant-authored emission on a controllable renderable surface enumerated by `.claude/CLAUDE.md` `## Constitutional Reporting Curtain` that fails any required Pre-Report Gate row, lacks explicit `REPORT-REASON` declaration when that law requires one, or matches non-reportable patterns per `.claude/reference/reporting-prohibition-law.md` `## Non-Reportable Content` is auto-classified `confirmed-defect:curtain-breach`. Promotion to `patch-worthy` requires the standard `confirmed-defect` -> `patch-worthy` promotion plus identification of the recurrence root cause (which constitutional invariant — (a) topmost / (b) independent / (c) priority / (d) always-maintained — failed for this emission). `curtain-breach` confirmed-defects are validator-hard-deny: any acceptance surface containing an unresolved `curtain-breach` confirmed-defect routes the verdict to `FAIL` (breach corrupts acceptance basis) or `HOLD` (correction can resolve within active acceptance boundary); `PASS` is prohibited regardless of other axes. Validator correction packet for `curtain-breach` names the constitutional invariant that failed and the smallest patch-ready correction.
...
- Verification depth requires substantive failure-mode probing of produced-result truth.
- Convenience-shaped output (per-axis enumeration, lens-step completion, defeater-list ticking, scope-coverage tally, or similar) without explicit disproof-attempt evidence on the most likely failure modes for the produced-result class is verification-shaped prose.
- Verification-shaped prose fails `Skill(self-verification)` Step 3 PASS-2, `Skill(review-verification)` `### 5. Critical Review Gate`, validator Final Acceptance Review, and every other producer outbound verification surface.
- Disproof-attempt evidence requires the named failure mode being probed, the observable evidence that would defeat the preferred positive claim if found, and the actual search record naming surface searched and finding.
- Shorthand (`tested`, `disproven`, `defeater enumerated`, `not material`, or similar) without these three components is verification-shaped prose and fails the same gates.
