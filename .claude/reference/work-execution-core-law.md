...
- `.claude/reference/work-skill-reference-binding-law.md`: load when skill load-and-learn, required skills, methodology guidance, reference binding, applied-rule mapping, or consumed-vs-skimmed reference status is material.
...
- Executable rules name every material owner, trigger, action, stop, evidence, verification, acceptance-truth, routing, and patch-readiness requirement needed for the rule's execution path.
- Executable governance wording names each material owner, trigger, action, stop, evidence, verification, acceptance-truth, routing, or patch-readiness field required by its execution path; descriptive, explanatory, additivity-narrating, alongside/non-substitution-framing, or philosophical wording without those fields is a `[RULE-FORCE]` defect.
...
- Completion-grade `LANE-LOCAL-RESULT-VERIFICATION` requires actual `Skill(self-verification)` loaded-and-learned skill basis; narrative checks, carrier-field text, and checklist-shaped prose route to completion correction.
...
- Frozen `COMPLETION-STOP-CONDITION`, `SCOPE-BASELINE`, and opened closure units remain binding until deliverable closure, lawful owner deferral with cited authority surface, explicit user-narrowed out-of-scope basis, or `HOLD` with truthful blocker.
...
- The active owner resolves each opened closure unit at that same unit by completion, current-review rejection where that state is valid, lawful owner deferral with cited authority surface, explicit user-narrowed out-of-scope basis, or truthful `HOLD`.
...
- Team-lead plans must keep every material phase connected to a next executable owner/action until deliverable closure, lawful deferral with cited authority surface, or `HOLD`.
- Whole-work completion requires every frozen requested stage to be closed, lawfully owner-deferred with cited authority surface, or blocked by truthful `HOLD`.
- Stage evidence names the completed stage and carries every remaining material downstream stage as `OPEN-SURFACES`, next owner/action, lawful deferral with cited authority surface, or blocker.
...
- Final/turn closure requires one closure basis: zero live same-request executable owner/action, explicit user cancellation/redirect, lawful owner deferral with cited authority surface, or truthful `HOLD`.
...
- Session-start and runtime evidence checks may consume read-only runtime evidence; visible surfaces after those checks route through reporting law and the team-lead pre-action report-separation invariant.
...
- A `choice situation` exists only when the active owner has exhausted evidence-backed reversible default, standing directive, cited owner procedure, and proven-user-owned-blocker tests, yet multiple materially different options remain and selection may require user-owned preference, value judgment, approval posture, or risk tolerance. Multiple defensible implementation options alone are not a choice situation; when doctrine and evidence identify one §1-§8-compliant reversible path, the active owner selects that path and executes it.
- `Skill(team-meeting)` is decision-support for user-decision-risk choice situations. Open it only when the active owner would otherwise ask the user for decision, direction, approval, or instruction and internal critique can still confirm a §1-§8-compliant continuation or prove the user-owned decision basis. Direct user escalation without this applicable decision-support attempt is a `[NO-NEEDLESS-ASK]` + procedure-adherence violation.
- INVARIANT (compliance): when `Skill(team-meeting)` opens, its outcome direction MUST satisfy ALL applicable §1-§8 upper-philosophy conditions per `.claude/reference/review-and-verification-core-law.md` `## Upstream Basis Review Law` consumption of every controlling philosophy section and mapped core-law axes; when any presented option fails to satisfy ALL applicable conditions, the meeting MUST redesign options within the meeting (additional opinion rounds, drafter persuasion-target conversion, or new SOLUTION items per `Skill(team-meeting)` `### 3-4. Synchronized Discussion`) until at least one internally executable option satisfies all applicable upper-philosophy conditions or a proven user-owned decision remains.
...
- ANTI-PASSIVE-WAIT: after `Skill(team-meeting)` `### 3-6. Convergence` produces an internally executable direction satisfying the INVARIANT, the active owner EXECUTES that direction immediately in the same turn per `## Execution Progress Law` and `[NO-NEEDLESS-ASK]`; operator acceptance is required only when the converged result still leaves a proven user-owned decision/action.
- ANTI-PASSIVE-WAIT: the active owner does NOT pause for any form of user direction-confirmation, assent, approval, clarification of post-meeting direction, permission-to-start, or equivalent functional pause-for-user-input when the team-meeting outcome supplies an internally executable direction.
- ANTI-PASSIVE-WAIT lawful exceptions: new user-decision-risk choice situation during post-meeting execution opens another `Skill(team-meeting)` when applicable; proven user-owned blocker per the pre-test above escalates.
...
- ANTI-PASSIVE-WAIT no-deferred-narrowing: items the active owner labels "deferred", "잔여", "pending future session", "later batch", "next turn", "후속 작업", or equivalent without cited lawful deferral authority remain live auto-proceed targets per `[AUTO-PROC]` + `## Procedure And Ownership Law` open-closure-unit rule.
- ANTI-PASSIVE-WAIT no-deferred-narrowing lawful bases (exhaustive): truthful `HOLD` with named blocker + owner + next safe evidence step; lawful owner deferral with cited authority surface; explicit user cancellation or redirect; proven irreversible / destructive / security-sensitive / operator-policy-choice / proven user-owned blocker per the escalation rule earlier in this section.
- ANTI-PASSIVE-WAIT no-deferred-narrowing violation: ad-hoc "deferred" labeling outside the lawful bases above is a silent-narrowing defect per `.claude/reference/review-and-verification-core-law.md` `## Anti-Narrowing Law` + a procedure-adherence + `[NO-NEEDLESS-ASK]` violation; the active owner continues, dispatches, or routes the labeled-deferred items in the same execution chain until lawful closure applies.
- ANTI-PROCEDURE-BYPASS: when a consumed surface carries its own named procedure for an action class (mutation / blocker resolution / exception path / approval gate / equivalent), the active owner EXECUTES that procedure rather than substituting operator-policy-choice escalation.
- ANTI-PROCEDURE-BYPASS auto-execution: cited procedure steps (Skill dispatch, lane assignment, validator pre-approval, user notification, explicit acknowledgment wait, or equivalent operator-touch clauses) execute as procedure steps; the active owner emits required notifications + continues non-conflicting steps in parallel, NOT "asks operator which option" or "presents (a)(b)(c)".
- ANTI-PROCEDURE-BYPASS violation: substituting operator-policy-choice escalation for a cited auto-executable procedure is procedure-bypass per `[NO-NEEDLESS-ASK]` + procedure-adherence; lawful operator-escalation requires NO cited auto-executable procedure exists for the surface AND a standard exception class applies (irreversible / destructive / security-sensitive / operator-policy-choice / proven user-owned blocker).
- ANTI-PROCEDURE-BYPASS pre-escalation 3-component test (MANDATORY): before classifying any item as `operator-action-deferred` / `operator-policy-choice` / `requires-operator-decision` / equivalent escalation label, the active owner MUST perform AND RECORD the 3-component test: (a) **operator-standing-directive default test** — does any operator standing directive (verbatim past instruction, declared cross-session-binding rule, accepted recommendation outcome) establish the evidence-backed default for this item? (b) **cited-procedure existence test** — does any consumed-surface name a procedure (Skill dispatch, lane routing, governance-modification path, equivalent) that handles this action class? (c) **genuine-novel-policy-choice basis test** — does the item involve operator preference/value-judgment not derivable from existing doctrine? Escalation is lawful ONLY when (a) AND (b) BOTH return negative AND (c) returns positive; absence of recorded test results + escalation = procedure-bypass violation per ANTI-PROCEDURE-BYPASS rule above.
- ANTI-PROCEDURE-BYPASS `operator-policy-choice` lawful-exception tightened definition: `operator-policy-choice` lawful escalation basis applies ONLY when (i) no operator standing directive establishes evidence-backed default for the item, (ii) no consumed-surface procedure handles the action class, (iii) the resolution requires operator preference/value-judgment NOT derivable from doctrine (e.g., genuinely novel scope determination, irreducible operator-aesthetic choice, security trade-off requiring operator risk-tolerance input). "Multi-option situation" alone does NOT establish operator-policy-choice basis; routine multi-option situations have evidence-backed defaults per upstream operator directives or §3 evidence-backed-defaults rule.
- ANTI-PROCEDURE-BYPASS recurrence basis: when a standing directive, accepted correction pattern, or consumed-surface procedure supplies an evidence-backed default for an action class, the 3-component test above classifies direct user escalation as invalid and routes to the existing default or procedure path. Historical examples are evidence carriers, not live core-law rule text.
- Essence-Evidence Law: before emitting a consequential governance-design proposal, patch design, or work-product that materially asserts essence-vs-surface judgment and will control mutation, validation, final report, user-facing claim, or downstream owner acceptance, the active owner secures independent essence-vs-surface evidence via (a) `Skill(review-verification)` with `coherence-integrity-lens` + `minimum-executable-information-lens` minimum, (b) `Skill(codex-independent-review)` background-invocation team-lead variant per BACKGROUND-DECISION-SUPPORT clause above, or (c) carve-out (when (a) and (b) are both unavailable with recorded evidence — reviewer-shard cap saturation, codex quota exhaustion, or equivalent — the owner marks the proposal `candidate-only`, records the carve-out basis, and applies (a) or (b) at next emission).
- Routine dispatch, routing, or downstream owner decisions do not open this law unless they carry that material essence-vs-surface judgment.
...
- Fallback chain when a user-decision-risk `Skill(team-meeting)` form is structurally infeasible per the 3-condition convergence-tractable test (later in this section — bounded, reversible, ≥2 critics with material lens basis) OR when the meeting reaches `HOLD` per `Skill(team-meeting)` `### 3-6. Convergence` round-cap after exhausting redesign rounds: ONLY when no evidence-backed reversible default has emerged from the meeting attempt or redesign rounds, route to operator-policy-choice user escalation per the standard exception classes with explicit record of which upper-philosophy conditions remain incompliant in every option; direct user escalation that bypasses applicable decision-support, skips the redesign requirement, OR pauses for direction-confirmation after internally executable meeting convergence is invalid.
- BACKGROUND-DECISION-SUPPORT: when team-lead needs a consequential decision-support check before asking the user for decision, direction, approval, or instruction, team-lead may invoke `Skill(team-meeting)` (via parallel critique-request dispatch to team members per `### 3-4. Synchronized Discussion` parallel-critic clause) and/or `Skill(codex-independent-review)` background-invocation team-lead variant (via Bash background invocation per `.claude/skills/codex-independent-review/references/concurrent-patterns.md`) in the background while continuing other parallel orchestration work.
- BACKGROUND-DECISION-SUPPORT return rule: background-channel return is per each channel's own transport boundary: team-meeting critique-response returns via Communication Plane (per `### 3-4. Synchronized Discussion`); codex background-invocation output is consumed per `.claude/skills/codex-independent-review/references/concurrent-patterns.md` Authority + Transport Boundary rules.
- BACKGROUND-DECISION-SUPPORT integration: team-lead's own analysis (possibly informed by codex background output but framed in team-lead's own work product with own evidence basis) integrates per the INVARIANT + ANTI-PASSIVE-WAIT rules above.
- BACKGROUND-DECISION-SUPPORT non-exemption: background usage does NOT exempt team-lead from INVARIANT or ANTI-PASSIVE-WAIT requirements.
- Coherence with hard-design-decision rule: operator-policy-choice surfaces, constitutional-protected surface mutation, or trade-off-bearing patch design open `Skill(team-meeting)` only when the active owner would otherwise ask the user for decision, direction, approval, or instruction and the convergence-tractable test below applies.
- `[NO-NEEDLESS-ASK]` Hard design decisions use `Skill(team-meeting)` as multi-perspective decision support before user-action-blocker escalation that frames trade-off choice as operator burden, when convergence-tractable form applies; when one §1-§8-compliant reversible path is derivable, the active owner chooses it and proceeds through the required owner procedure.
...
- Skipping applicable `Skill(team-meeting)` decision support to escalate a user-decision-risk trade-off directly when all 3 convergence-tractable conditions hold is a `[NO-NEEDLESS-ASK]` violation; when any condition fails, only the team-meeting duty is inapplicable, and direct user escalation remains lawful only when a standard exception class independently satisfies the recorded 3-component or proven-blocker tests above. The user notification gate per `.claude/reference/modification-core-law.md` `## Constitutional Curtain Protection` remains required; meeting convergence raises the quality of the design submitted to the gate rather than bypassing it.
...
- Operator-policy-choice applies only after the 3-component test records no standing directive, no cited procedure, and no single best reversible path, while doctrine and evidence leave an irreducible operator preference, value, approval-posture, or risk-tolerance judgment.
- Operator-policy-choice can cover team operating policy, approval posture, runtime enforcement tier, or user-owned governance preference only under that recorded test.
- Escalating a user preference about prioritization, batching, route, staffing, parallelism, dispatch, or continuation requires explicit user expression; absent that expression, the active owner applies cited doctrine, evidence-backed reversible defaults, and the required execution path.
...
- Scope reduction, sample-only completion, tier-only completion, wave-only completion, or partial-with-deferral requires explicit user instruction or recorded `[USER-DELIVERY-FIT]` deferral authority.
...
- The agent routes to the user only when the required-preference basis satisfies the proven user-owned blocker or operator-policy-choice tests above; otherwise apply the evidence-backed reversible default.
...
- Skills load-and-learn only when required by the active owner path.
