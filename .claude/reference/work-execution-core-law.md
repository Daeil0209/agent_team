...
- Named owner procedures execute before user escalation when they can resolve the action class.
- Before asking the user for decision, direction, approval, or instruction, open `Skill(team-meeting)` when internal critique can still derive a §1-§8-compliant reversible continuation or prove the user-owned blocker.
- The active owner records a compact escalation basis only when no standing directive, no cited owner procedure, and no evidence-backed reversible path resolves the user-owned choice.
- Labels such as "deferred", "pending", "later", "operator-policy-choice", or equivalent do not close opened work unless backed by truthful `HOLD`, lawful owner deferral with cited authority, explicit user cancellation/redirect, or a proven user-owned blocker.
- Governance-design, patch-design, and work-product claims use their existing owner gates (`Skill(review-verification)`, `Skill(self-verification)`, `Skill(governance-modification)`, validator) rather than a separate decision-support or essence-evidence layer.
...
- Status, count, yes/no, verification, or explanation questions are reporting or audit requests; continuation, route, staffing, parallelism, dispatch, validation, cleanup, artifact mutation, and governance asset mutation require matching execution path.
...
- The cap basis records standing operator maximum 2, explicit lower user maximum when present, current runtime/session ceiling when present, host capability limit, and default basis.
...
- Runtime status, disk output, pane text, final prose, and visible teammate output remain supporting signals only.
...
- Same-turn next-action drive applies to every direct-execution step (`Edit`, `MultiEdit`, `Write`, `Bash`, `SendMessage`, `TeamCreate`, `Agent`, every mutation, every dispatch, every tool call).
...
- Planning, dispatch, readiness, and recovery measurements such as file counts, line or byte counts, runtime/team status counts, carrier checks, and readiness counts are not user-facing evidence while the same-request next owner/action can continue from retained evidence. Route them to retained/internal records unless the user explicitly requested exact raw output or the measurement itself is the only blocker-clearing evidence.
...
- Route internal detail to governed carriers, artifacts, retained outputs, or quiet command results according to `.claude/reference/reporting-prohibition-law.md`.
...
- `Bash(sleep N; <check-or-output>)`, chained sleeps, and sleep-delayed filesystem/runtime polling are invalid waiting primitives. Waiting for a condition uses the governed Monitor/until-loop owner path; waiting for a command started by the agent uses the tool's background-process path; waiting for dispatched agents uses the runtime monitoring owner. Do not use `sleep` plus `ls`, `cat`, `echo`, `ps`, `stat`, or equivalent probes as a dispatch, completion, or progress check.
...
- A `Bash` command that may exit non-zero runs outside the same batch as critical sibling tool calls whose cancellation would interrupt mutation, dispatch, or communication (`Edit`, `MultiEdit`, `Write`, `SendMessage`, or equivalent).
...
- `[PROJECT-OUTPUT-ROOT]` Work outputs stay under the active project output folder.
