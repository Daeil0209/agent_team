...
- ANTI-PROCEDURE-BYPASS auto-execution: cited procedure steps (Skill dispatch, lane assignment, validator pre-approval, user notification, absence-of-objection wait, or equivalent operator-touch clauses) execute as procedure steps; the active owner emits required notifications + continues steps in parallel, NOT "asks operator which option" or "presents (a)(b)(c)".
