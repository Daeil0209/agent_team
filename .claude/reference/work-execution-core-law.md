...
- ANTI-PASSIVE-WAIT no-deferred-narrowing: items the active owner labels "deferred", "잔여", "pending future session", "later batch", "next turn", "후속 작업", or equivalent without a lawful deferral basis remain live auto-proceed targets per `[AUTO-PROC]` + `## Procedure And Ownership Law` open-closure-unit rule.
...
- ANTI-PROCEDURE-BYPASS: when a governance surface, mapped core-law section, skill body, hook script, or other consumed surface carries its own named procedure for handling a class of action (surface mutation, blocker resolution, exception path, escalation flow, recovery step, approval gate), the active owner EXECUTES that cited procedure rather than substituting operator-policy-choice escalation.
- ANTI-PROCEDURE-BYPASS auto-execution semantics: cited procedure steps include any required `Skill(...)` dispatch, lane assignment, validator pre-approval, user notification, wait-for-absence-of-objection, or equivalent operator-touch clauses; the active owner emits the notification + continues procedure steps in parallel, NOT "asks operator which option" or "presents (a)(b)(c) for operator decision".
- ANTI-PROCEDURE-BYPASS violation: substituting operator-policy-choice escalation for an auto-executable cited procedure is a procedure-bypass defect per `[NO-NEEDLESS-ASK]` + procedure-adherence rule; lawful operator-escalation requires that NO cited auto-executable procedure exists for the surface AND one of the standard exception classes applies (irreversible / destructive / security-sensitive / operator-policy-choice / proven user-owned blocker bases per the escalation rule earlier in this section).
...
- BACKGROUND-DECISION-SUPPORT: when team-lead needs any consequential decision (choice situation per above OR other consequential decision requiring evidence/critique), team-lead may invoke `Skill(team-meeting)` (via parallel critique-request dispatch to team members per `### 3-4. Opinion Round` parallel-critic clause) and/or `Skill(codex-independent-review)` background-invocation team-lead variant (via Bash background invocation per `.claude/skills/codex-independent-review/references/concurrent-patterns.md`) in the background while continuing other parallel orchestration work.
- BACKGROUND-DECISION-SUPPORT return rule: background-channel return is per each channel's own transport boundary: team-meeting critique-response returns via Communication Plane (per `### 3-4. Opinion Round`); codex background-invocation output is consumed per `.claude/skills/codex-independent-review/references/concurrent-patterns.md` Authority + Transport Boundary rules.
