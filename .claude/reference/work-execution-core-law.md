...
- ANTI-PASSIVE-WAIT: after `Skill(team-meeting)` `### 3-6. Convergence` + `### 3-7. Operator Gate` (operator-accept closes the meeting) produces a converged direction satisfying the INVARIANT, the active owner EXECUTES that direction immediately in the same turn per `## Execution Progress Law` ("Process state report opens the next truthful action in the same turn" + "The agent keeps emitting required tool calls while live same-request executable owner/actions remain") and `[NO-NEEDLESS-ASK]` ("Choose the evidence-backed best solution" earlier in this section).
- ANTI-PASSIVE-WAIT: the active owner does NOT pause for any form of user direction-confirmation, assent, approval, clarification of post-meeting direction, permission-to-start, or equivalent functional pause-for-user-input.
- ANTI-PASSIVE-WAIT: the team-meeting outcome IS the direction.
- ANTI-PASSIVE-WAIT: encountering a new choice situation per the trigger rule above during post-meeting execution lawfully opens another `Skill(team-meeting)` and is NOT a passive-wait violation.
- ANTI-PASSIVE-WAIT: pausing for a user-owned blocker per the pre-test above lawfully escalates and is NOT a passive-wait violation.
- ANTI-PASSIVE-WAIT: passive waiting for user direction-confirmation post-meeting (any pause for user assent/approval/confirmation/permission where the team-meeting-converged direction is sufficient to execute) is a `[NO-NEEDLESS-ASK]` + procedure-adherence violation.
- ANTI-PASSIVE-WAIT scope: the "direction" governed by ANTI-PASSIVE-WAIT is the INVARIANT-verified team-meeting convergence outcome per the INVARIANT (compliance) bullet above.
- ANTI-PASSIVE-WAIT scope: executing pre-convergence un-verified next-action is a DIFFERENT defect class (premature-execution per §4 Review And Verification Philosophy).
- ANTI-PASSIVE-WAIT scope: premature-execution is NOT a fulfillment of ANTI-PASSIVE-WAIT.
- ANTI-PASSIVE-WAIT scope: verification-of-next-action is UPSTREAM of ANTI-PASSIVE-WAIT in the execution order.
- ANTI-PASSIVE-WAIT scope: fast un-verified action ≠ ANTI-PASSIVE-WAIT compliance.
