...
- Internal `Bash` command text, tool parameters, stdout, or stderr that can render to the terminal is a display-output surface; `.claude/reference/reporting-prohibition-law.md` owns admission, and this section owns the executable runtime/tool-output mechanics.
- Internal `Bash` command text and parameters stay concise and free of progress narration, route rationale, labels, section separators, carrier contents, or user-facing explanations unless the user explicitly requested that exact command material and report admission passes.
- Normal read-only inspection/search pipelines and short inline shell logic are valid when they are the truthful evidence path; route their stdout/stderr through the quiet or file-backed shapes below.
- Evidence-bearing internal `Bash` commands keep terminal stdout and stderr empty unless the user explicitly requested that exact command output or the reporting law admits that exact material.
- Before executing evidence-bearing internal `Bash`, select one lawful command shape: silent predicate with stdout/stderr empty and routing by exit status; file-backed capture that writes stdout/stderr to a governed retained artifact, task state, runtime ledger, or evidence artifact for later owner consumption; or admitted output for exact user-requested/report-admitted command material.
...
- Screen suppression preserves agent evidence. When the acting owner or downstream owner needs command detail beyond exit status, capture that detail in a governed retained artifact, non-rendered task state, runtime ledger, or evidence artifact before suppressing terminal stdout/stderr; do not use empty terminal output or `No output` as the sole evidence path for a content-dependent decision.
- Bash detail needed for packet assembly, sharding, carrier verification, monitoring, synthesis, correction, proof, or validation remains available to the responsible owner through the governed capture path and remains absent from user-rendered stdout/stderr unless report admission or exact user-requested command output applies.
- Internal inventory, measurement, excerpt, settings-inspection, sharding, dispatch-preparation, carrier-verification, monitoring, or synthesis commands do not print path lists, counts, tables, excerpts, labels, pass/fail facts, section separators, or progress sentinels to the terminal.
- Expected false workflow states such as missing not-yet-produced carriers, no grep match, empty pending files, or count zero are routed through quiet predicates, internal state, retained artifacts, correction, waiting, reuse, or synthesis decisions without terminal stdout/stderr and without surfacing as a `Bash` error.
- Do not use hook-side display suppression for evidence-bearing `Bash` output unless live runtime proof shows the acting agent still receives the completed command result. Observed `suppressOutput` behavior can make worker-visible `Bash` results appear as `No output`, so it is not a valid screen-safety control for evidence, connectivity probes, inventories, excerpts, counts, or progress sentinels.
...
- If an internal command accidentally prints non-reportable content, classify the incident as a renderable-projection violation, preserve evidence only from governed artifacts or rerun a corrected quiet capture when needed, and continue the owner action without narrating the noisy command.
