...
- Positive-pattern, doctrine-shape, or compliance-field enforcement is invalid hook body content per `[HOOK-LAST]` and routes to the lane trio (`Skill(governance-modification)` + `Skill(self-verification)` + `Skill(review-verification)` named lenses) for correction unless the hook is an enumerated constitutional reporting-curtain runtime guard.
