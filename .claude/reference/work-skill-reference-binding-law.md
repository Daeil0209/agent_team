...
- A `Skill(<skill>)` load, reload, same-session reuse, trigger-reference load, or owner/action transition boundary creates no assistant-authored report slot. The next assistant-authored renderable candidate after that boundary must first consume current `.claude/reference/reporting-prohibition-law.md` applied-rule mapping for that exact surface.
...
- `.claude/reference/reporting-prohibition-law.md` is trigger-active for every assistant-authored renderable surface before that surface is constructed.
- Reporting-prohibition consumption cannot be `not-material` for a candidate renderable surface.
- Reporting-prohibition consumption requires current applied-rule mapping to the exact candidate surface: prose body, pane/final text, renderable tool or channel field, `SendMessage` visible field, task-plan text, `Bash` command text, `Bash` tool parameters, stdout, or stderr.
