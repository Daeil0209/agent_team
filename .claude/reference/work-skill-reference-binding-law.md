...
- A `Skill(<skill>)` load, reload, same-session reuse, trigger-reference load, or owner/action transition boundary creates no assistant-authored report slot. The next assistant-authored renderable candidate after that boundary first consumes `.claude/reference/reporting-curtain-spine.md`; if the spine escalates or cannot classify the candidate as routine suppression, consume current `.claude/reference/reporting-prohibition-law.md` applied-rule mapping for that exact surface.
- When that mapping denies prose, the replacement action is the active or loaded owner's next governed `Read`, tool call, carrier/file operation, dispatch, verification, correction, or blocker-routing move.
- Do not write skill-load success, startup-complete, no-active-user-work, reference-loading, binding-surface-authoring, runtime-creation, materialization, readiness, or next-step bridge prose after a skill load, trigger-reference load, or owner/action transition unless reporting-prohibition admits that exact surface.
...
- `.claude/reference/reporting-curtain-spine.md` is trigger-active for every assistant-authored renderable surface before that surface is constructed.
- `.claude/reference/reporting-prohibition-law.md` becomes trigger-active when the spine escalates, cannot classify routine suppression, or any report-admission candidate exists.
- Reporting-curtain consumption cannot be `not-material` for a candidate renderable surface.
- Reporting-curtain consumption requires current applied-rule mapping to the exact candidate assistant-authored renderable content surface: prose body, pane/final text, renderable tool or channel field, `SendMessage` visible field, task-plan text, or assistant-authored prose/detail embedded in command labels, command-generated text, or channel payloads.
- A startup `REPORTING-BASIS`, path citation, tool-row evidence, previous report denial, or remembered rule is only a reusable source basis; it is not consumption for a new renderable surface until the exact applied-rule mapping is current.
- Same-session consumed-reference basis is reusable only while the loaded surface still supplies the needed rule, the applied-rule mapping still matches the current owner/action, and session, file, compaction/drop/truncation, or unconsumed trigger-detail staleness is absent.
- A stale reusable basis opens refresh of the stale complete document or complete triggered section, not repeated consumption of unrelated already-current documents.
