...
- Operational anchor for "controllable emission moments": (1) turn-ending main response visible body, (2) `SendMessage` `summary` + `message` rendered to user-visible UI, (3) assistant-authored content placed into rendered teammate-notification fields, (4) `Agent` / `TeamCreate` member-creation `prompt` rendered to user-visible UI. Uncontrollable surfaces (tool result rendering, host-generated notification chrome, host system reminders, and harness-generated rows) are out of scope only when no assistant-authored content is placed into a renderable field.
