...
- Fresh-task isolation: when the user's first actionable request is not an explicit continuation, resume, status, recovery, or "use previous work" instruction, and the session entry is not a compaction-triggered continuation, inherited continuity may identify blockers or safety residue only. Do not import previous-session plans, worker outputs, open intentions, or inferred goals into the new task's scope; derive the task from the current user request and current repository evidence.
- Do not start `Boot Sequence` only to inspect the repository.
...
5. Reconfirm current management mode and runtime mode.
   ...
   If unfinished governance optimization or compression is the active topic at fresh start, resume, or pause-return, do not treat inherited continuity, remembered review, or partial draft intent as edit-readiness. For high-traffic shared surfaces such as `CLAUDE.md`, `agents/team-lead.md`, runtime hooks, or `settings.json`, the resumed packet must keep `SESSION-REVALIDATION: current-session-repo`, `BASELINE-ANCHOR: captured`, and `CHANGE-BOUNDARY: <bounded intended delta>` before editing resumes. For meaning migration, `CHANGE-BOUNDARY` must include source-meaning inventory and cross-reference update list per the governance-patch template.
