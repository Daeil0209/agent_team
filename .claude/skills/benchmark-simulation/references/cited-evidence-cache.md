...
- Anti-pattern: E2E-heavy suites raise runtime, flakiness, and debugging difficulty proportional to scope. Flaky failures erode trust in passing tests. Doctrine that requires the most faithful surface for every check can create an execution burden that causes verification to be skipped; skipped verification is worse than narrowed-scope verification. `[USER-SURFACE]` needs SMURF-Reliability balance.
