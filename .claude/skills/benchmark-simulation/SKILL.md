...
## Authority
**This lens covers:** benchmark framing, fairness notes, baseline choice, comparison axes, regression visibility, and lean-team choice.
...
- **Principle-class references**: Material/Carbon/HIG/Polaris design principles, Nielsen Norman heuristics, design-system identity declarations.
  ...
  They must enter the abstraction-fit-adapt cycle as input.
...
It is not required when prior session already froze the verification basis from external evidence or when tester/validator uses an already-frozen proven local default cited in the packet, lane rule, or acceptance surface.
...
Decision-ready output requires cited comparison basis, fit judgment, exercised axes, and residual risk or open surfaces.

### Pattern Lifecycle Staging
Each durable pattern catalog entry carries exactly one lifecycle state:
...
Single-axis selection, single-source reliance, or single-modality observation is incomplete when the decision depends on more than that axis.
Each decision-ready benchmark records which comparison axes were exercised, which were skipped, and why.
Silent axis skip blocks `decision-ready`.
## Inputs
- active request source or canonical request packet
- current role model and governing doctrine
- upstream comparative evidence when available
