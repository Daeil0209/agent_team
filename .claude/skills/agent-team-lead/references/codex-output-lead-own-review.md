...
- Lawful fail-open applies only when the configured-review governing reference (`.claude/skills/codex-independent-review/SKILL.md` `## Review Workflow` step 8 + `.claude/skills/codex-independent-review/references/common-contract.md` `## Tool Discipline` fail-open enumeration + `## Handling` retry-then-fail-open rule) explicitly names the fail-open path for the current review type.
