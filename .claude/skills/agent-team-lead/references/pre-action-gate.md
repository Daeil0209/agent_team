...
- Renderable-prose invariant: ordinary assistant message, pane, final, task, tool, and channel text authored by the assistant is a controllable renderable surface; if reporting law denies it, the only valid continuation is the next governed owner action or an empty/omitted/single-space renderable field.
...
- Command-output invariant: internal-evidence `Bash` commands follow `.claude/reference/work-runtime-boundary-law.md` `### Bash Internal Evidence Capture Contract`; expose only user-requested command output after report admission.
