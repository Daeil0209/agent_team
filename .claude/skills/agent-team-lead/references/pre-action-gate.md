...
- Report separation invariant: visible-prose surfaces, non-reportable content classes, command-stdout admission, admitted status prose, and report shape are owned by `.claude/reference/reporting-prohibition-law.md` `## Consume When`, `## Pre-Report Gate`, `## Non-Reportable Content`, and `## Report Shape`.
