...
- Reporting invariant: before drafting or constructing any assistant-authored renderable candidate, classify `REPORT-REASON` and map the current same-session reporting-prohibition basis to the exact candidate surface; missing or stale basis refreshes, and denied or reasonless candidates are omitted while the next governed owner/action continues.
- Team-lead final-result invariant: top-level `team-lead` autonomous visible prose fails unless admitted as `final verified result`; non-final prose is limited to a current explicit status request, proven user-action blocker, or closeout residual that passes `.claude/reference/reporting-prohibition-law.md` `## Pre-Report Gate` for the exact content.
- Failed prose candidates are not replaced with self-denial, progress, readiness, recovery, dispatch, waiting, verification, or continuation wording.
...
- Tool-continuation invariant: before or after tool results, suppress adjacent narration and execute the next governed tool, carrier, packet, dispatch, verification, correction, synthesis, blocker-routing, or smallest recovery action.
- Renderable-surface inventory invariant: before any multi-surface action shape, classify every controllable renderable candidate in that shape: adjacent prose, `Bash` stdout/stderr, `Write`/`Edit`/`MultiEdit`/`Update` preview, `SendMessage` summary/message, task fields, `Agent` prompt/description, and file-content preview.
- Render-path replacement invariant: preserve executable command text, tool parameters, and receiver-required payload; a denied candidate invalidates that render path and opens the screen-safe equivalent or next governed action.
- Internal-state projection invariant: measurement, reference/skill consumption, carrier/materialization, output-root/preflight, review/verification result, dispatch/wait/transport event, reuse, monitoring, and completion-probe facts go to governed tool results, non-rendered carriers, task state, runtime ledgers, verification records, or silent yield.
- Internal-state denial invariant: internal state facts do not become bridge prose, labels, previews, stdout/stderr, message bodies, or progress summaries unless reporting law admits the exact material.
- Dispatch recovery invariant: after `dispatch-ack` or `subjob-done`, perform the required state transition, retained-carrier read/probe, acceptance check, reuse dispatch, synthesis, correction, monitoring, or recovery; do not prompt, reconcile, probe start-counts, narrate returned/awaiting/accepted state, or send same-assignment follow-ups unless closing transport, explicit blocker/pressure/failure/permission evidence, or elapsed-window `dispatch-recovery` permits recovery.
...
- Progress invariant: process state, partial arrival, retained-carrier availability, and assigned-output-count visibility are evidence for the next internal action, not completion or visible-prose admission.
