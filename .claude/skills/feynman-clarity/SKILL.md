...
- Fixed skill pattern:
  ...
  4. skill-local structural feedback and governance modification blocks when present
- Keep this skill methodology-only.
- Coverage stays limited to clarity.
...
- Codex consumes this lens only through routed review packets.
...
**Adjacent boundaries:** canonical planning, factual research, domain truth, implementation fixes, test execution, rendered visual proof, and final validation verdict.
...
- map each important term to its role, evidence, or action
...
- classify unresolved gaps by routing path instead of inventing unsupported meaning
...
- Evidence review, technical correctness review, and user-surface proof stay with their active paths.
...
- active lane or next path/action when the issue is not clarity-only

## Outputs
Capture only the decisive Feynman-clarity packet:
- `CLARITY-TARGET`
- `TARGET-READER`
- `BROKEN-EXPLANATION`
...
Use `INFERENCE` only when reconstructed from context without inventing scope, closure, acceptance, source basis, or reader/operator intent.
...
8. Remove repeated wording when one direct sentence can carry the meaning.
9. Rewrite only after the meaning, evidence, and stop condition are clear.
10. Compare the rewrite against the source meaning.

If source meaning cannot be preserved, hold or route the ambiguity.
Route the gap to the narrowest correction path.
...
- the next path/action when the document is not final
...
If two places say the same thing, keep the narrowest consumed surface and replace the other with a pointer or remove it.
...
- the next path/action for each remaining gap
- whether the artifact is clearer without weaker claims

Report `hold|blocker` when the artifact cannot be explained without inventing meaning, hiding missing evidence, or deleting a required obligation.

## Codex Review Consumption
Codex applies this skill only as a review lens inside a routed packet.
The packet preserves Codex's evidence-only boundary.
...
- Does it name the task, completion path, evidence path, routing path, and acceptance surface?
...
The gap is a planning, review, implementation, proof, or validation risk according to routing path.
...
- Choose the best evidence-backed rewrite or correction route for ordinary wording choices.
- If the clarity finding changes scope, route, acceptance, or claim strength, return that pressure to the governing procedure.
...
- State the smallest rewrite or correction route that preserves meaning.
