...
## Team-Routing Guard
If `team-routing candidate` is frozen, carry required non-lane-core skills from `LANE-REQUIRED-SKILLS-MAP`.
If `ambiguous-route` is frozen, carry required non-lane-core skills from lawful phase-local refinement.
Lane ownership triggers lane-core loading.
Packet skill listing carries non-lane-core required skills.
