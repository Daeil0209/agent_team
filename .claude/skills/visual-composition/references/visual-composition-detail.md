...
- **Merged-Cell Layout Prohibition**: Block spreadsheet-style merged-cell translation in web UI when it visually flattens distinct row/group identity, breaks responsive inspection, hides controls, or forces manual scanning that a structured table, grouped header, or workflow panel should preserve.
- Route semantic data-relationship uncertainty to `business-workflow` or `software-architecture` instead of deciding it by visual composition alone.
- Block mobile-first layouts when desktop operators lose visible controls, comparison density, or repeated-action efficiency.
