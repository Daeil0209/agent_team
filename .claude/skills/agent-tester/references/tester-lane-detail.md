...
- If required postcondition or Playwright-equivalent evidence cannot be produced because the tool, runtime, or execution capability is unavailable after safe inference and authorized setup/tool routes, send `MESSAGE-CLASS: hold|blocker` with the common tool/evidence-gap fields.
