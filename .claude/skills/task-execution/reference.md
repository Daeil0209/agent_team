...
| Structural | Multi-file, new sections, meaning migration, architecture | Standard + BASELINE-CLASS + EDIT-OPERATION. For migration: CHANGE-BOUNDARY must include source-meaning inventory + destination rationale + cross-reference list |
...
Dispatch-proof hook enforcement is advisory for ordinary wording drift and for non-core detail gaps on lanes where the lane, intent, and safe boundary remain inferable. That advisory rule does **not** relax hard requirements for `tester` or `validator` core contract fields, and it does **not** relax the same-line `KEY: value` requirement for those fields. For `tester` and `validator`, missing core contract fields remain blocking at dispatch time; worker reconstruction begins only after those core fields pass hook-level validation. Outside that strict-core case, missing or approximate clean-packet fields should not by itself block `Agent` dispatch when the lane, intent, and safe boundary are inferable. The receiving worker must reconstruct the working packet explicitly before execution, and must return `MESSAGE-CLASS: hold` when missing fields create material ambiguity in scope, authority, proof target, acceptance basis, or lifecycle.
...
For workflow-governed discovery (for example `dev-workflow` Phase 0), default `DOWNSTREAM-CONSUMER` is `team-lead (CP1 synthesis)`, not the later plan writer directly. In that path, default `OUTPUT-SURFACE` is a handoff summary or evidence packet; do not require a worker-written project artifact unless the workflow explicitly assigns a write-compatible artifact owner for the research lane. If one packet mixes mechanical inventory or source extraction with 2+ independent interpretive surfaces, it is not `bounded`; use `sharded` or a scout-then-shards path instead.
When planning hands forward `DISCOVERY-SPLIT-SHAPE`, keep the researcher packet aligned to that basis instead of flattening it back into one broad researcher by habit. `single` is valid only for one bounded discovery surface; if the inherited basis still mixes extraction/inventory with 2+ interpretive surfaces, require `scout-then-shards` or `immediate-shards`.
...
Sharded adds: `SHARD-ID: <id>; SHARD-BOUNDARY: <boundary>; MERGE-OWNER: <owner>`
...
- In the actual dispatch packet, each required tester field must appear as same-line `KEY: value`. Keep expanded scenario bullets under `DETAILS` or another non-required section after the scalar summary.
For executable, user-facing deliverables:
...
- Keep tester packets on proof execution. Do not silently bundle integration-ready closure, environment bring-up, browser proof, and final Phase 4 synthesis into one mixed-purpose tester dispatch. If readiness is still unresolved, route a separate readiness follow-up first (implementation follow-up, lead-local closure, or one bounded `tester-env` packet), then dispatch browser proof against the integrated output.
...
- In the actual dispatch packet, each required validator field must appear as same-line `KEY: value`. Keep expanded rationale bullets under `DETAILS` or another non-required section after the scalar summary.
For executable, user-facing deliverables:
...
| tester/validator | BURDEN-CONTRACT | hands-off or low-touch or normal or not-applicable |
...
Hard stop: if any completed worker on the same work surface lacks an explicit lifecycle decision, no `Agent`, `TaskCreate`, or assignment-grade `SendMessage` dispatch is allowed for that surface. Dispatch to a different, non-overlapping work surface is not blocked by unrelated completed workers, provided those workers have a pending lifecycle decision queued for the next available processing cycle. Resolve that backlog first even when the next work is urgent, corrective, or parallelizable. Recovery: send each undecided worker a lifecycle `SendMessage`.
...
1. Complete fresh-turn work-planning scope freeze.
2. Complete post-planning self-verification.
3. Classify the outgoing action as `Agent`, `TaskCreate`, or assignment-grade `SendMessage`.
4. Resolve lifecycle backlog: every completed worker **on the same work surface** must have an explicit `reuse`, `standby`, `shutdown`, or `hold-for-validation` decision before dispatching to that surface.
...
4. These tools are administrative state-channel updates, not deliverable execution, but on a fresh user turn they are still consequential mutation. Required order on a fresh turn: current-turn `work-planning` -> post-planning `self-verification` -> exact task-id validation -> task-state mutation. Within an already-open verified execution segment, exact-id validation remains the tool-specific preflight before mutating task state.
...
- Team-runtime truth uses 3 independent checks: `team exists`, `dispatch pending`, and `worker started`. Keep them separate in both reasoning and reporting.
...
- Routing decision rule: if canonical team-existence evidence is absent, `TeamCreate` with a collision-free session-scoped team name is the next consequential action. A semantic team-name collision with existing config residue is not a reason to delete by default; choose a unique team name unless the active session is already in an explicit stale-runtime cleanup path. If canonical team-existence evidence is present and a suitable live or standby worker already exists, prefer assignment-grade `SendMessage` reuse. If canonical team-existence evidence is present and no suitable worker exists, use `Agent`. If dispatch-pending evidence exists without worker-start evidence, monitor or diagnose the pending state instead of reissuing `TeamCreate` or narrating the work as active.
...
- `worker-started`: `dispatch-ack`, `lastClaimedWorker` match, first worker tool use, or worker-originated `status|handoff|hold|blocker|completion`. `dispatch-ack` classifies the worker as working until completion-grade output and a lifecycle decision prove otherwise.
...
- `working-permission-pending`: worker-start evidence exists and the worker is awaiting user approval for a tool request. Treat the worker as working; resolve the permission prompt, do not status-probe the worker.
- `working-report-missing`: a turn-ended runtime signal arrives after worker-start evidence without completion-grade output, blocker, or permission request. Treat as a reporting gap on a working worker, distinct from unclaimed dispatch and not proof that the worker is not working.
- A bounded monitoring `SendMessage` to an already-started worker is non-assignment coordination when it only asks for status, clarification, or the smallest partial result, carries no `REQUIRED-SKILLS`, and carries no `LIFECYCLE-DECISION`. This coordination may proceed without reopening the fresh-turn dispatch lock. It does not apply to `dispatch-pending`, `ack-late`, `working-permission-pending`, standby, or completed-awaiting-lifecycle targets.
...
- Lifecycle preflight before spawning: resolve all completed workers awaiting lifecycle on the same work surface before dispatch. Reuse existing workers before spawning replacements. Worker completion creates an immediate lifecycle obligation; turn-ended runtime signals without completion keep the worker in working state.
...
## Dispatch Packet Final Check Required Fields
...
- `dispatch-proof-gate` warns on ordinary wording drift, but it blocks missing core `tester`/`validator` contract fields. Do not rely on worker reconstruction for those fields.
- Field headers with empty values are parsed as missing by hook-level extraction. If detail needs bullets, keep a one-line scalar value on the field line and move the expanded bullets under `DETAILS` or another non-required section.
...
- Orchestration channel order is fixed: (1) answer the user's interrupt or status question from current evidence, (2) execute any required worker lifecycle decision, (3) perform dispatch or reuse, and only then (4) mutate shared task state if needed. A later channel never substitutes for an earlier one: status text does not resolve lifecycle, lifecycle does not complete a dispatch packet, and task-state mutation does not consume a workflow cursor or authorize new work by itself.
...
- Lifecycle backlog means completed workers without an explicit `reuse`, `standby`, `shutdown`, or `hold-for-validation` decision. Treat this state as `completed-awaiting-lifecycle` or `lifecycle backlog`, not as standby and not as a working worker's turn-ended signal. The target is zero undecided completed workers before additional fan-out, not zero turn-ended signals.
- Corrective work and bottleneck fixes still obey lifecycle order. When a new fix is needed and completed workers exist, the next action is lifecycle resolution or bounded reuse, not a fresh `Agent` spawn.
...
- Task-state mutation preflight is shared by `TaskUpdate` and `TaskStop`: on a fresh user turn, complete current-turn `work-planning` -> post-planning `self-verification` first, then confirm the exact task id from `TaskList`, `TaskGet`, or the original `task_assignment` packet before mutating task state. If the id is stale or absent, do not guess; report administrative task-state unavailable. These tools remain administrative state-channel updates, not deliverable execution; inside an already-open verified execution segment, exact-id validation is the remaining tool-specific preflight before mutation.
...
- SendMessage direction: downward authoritative control uses MESSAGE-CLASS, MESSAGE-PRIORITY, WORK-SURFACE; lifecycle shutdown uses shutdown_request / shutdown_response; upward authoritative reports use message-class, message-priority, work-surface, requested-governing-action; peer worker challenge uses PEER-MODE + MESSAGE-PRIORITY. Free-form is fine for status, acknowledgment, clarification, partial-result notes, or inferable assignment context that does not create material ambiguity in ownership, lifecycle, routing, or active surface. Downward free-form status/clarification/partial-result requests to already-started workers are monitoring coordination, not assignment-grade dispatch.
- Direct worker communication after assignment must preserve the worker's active phase intent and owned boundary. Status, clarification, partial-result, reuse, and lifecycle messages may refine or monitor the current owned surface, but must not silently widen the worker into adjacent phase work, cross-lane evidence closure, or lead-owned synthesis. If the next needed work materially changes phase intent, owned surface, or return form, freeze a new planning/dispatch basis and issue a new packet instead of smuggling the change through follow-up or lifecycle traffic.
...
- Lifecycle control messages are lifecycle-only. Keep any natural-language body empty or limited to one short `DECISION-BASIS` sentence. Do not append handoff recap, renewed review scope, residual-finding summary, future reroute/recall hints, or any wording that could plausibly be read as a new assignment or re-opened judgment surface. If new work, renewed review, or further evidence action is actually intended, send a separate assignment-grade packet instead of overloading lifecycle control.
...
- Do not require one lane's handoff to close another lane's evidence surface on behalf of the owning lane. Each lane packet must stay on lane-owned evidence. When another `reviewer`/`tester`/`validator` owner is already assigned, missing downstream review, proof, or verdict evidence must be routed or held rather than backfilled inside the current lane; tree-wide grep counts, cross-reference zero-occurrence audits, repository-wide diff proof, replacement proof gathering, and final verdict closure stay with the owning lane. If that evidence is needed before routing, review it lead-local before dispatch instead of inflating the current lane handoff.
...
- When the active workflow expects verification continuity across phases, that follow-on derivation must preserve the upstream row-mapped coverage/delta block for the relevant plan, design, success-criteria, or proof rows. If the returned handoff does not make row state, stale rows, deferred rows, or assumption deltas explicit enough for the next lane, do not inflate the downstream packet to compensate. Either synthesize that continuity lead-local from stronger same-surface evidence, or request one bounded follow-up from the owning lane to complete the missing continuity rows before routing onward.
...
- `AGENT-MAP` and `PARALLEL-GROUPS` together are the staffing basis handed forward from planning, not casual notes. `task-execution` remains the staffing owner at dispatch time and must either operationalize that basis or name the concrete runtime, lifecycle, overlap, blocker, or scope reason for changing it.
- When `PARALLEL-GROUPS` records `none — <serial reason>`, task-execution may keep staffing narrower on that recorded dependency, boundary-uncertainty, or context-cost basis, or on a newly surfaced runtime truth, blocker, overlap/reuse constraint, or scope change. Do not silently rediscover staffing from scratch at dispatch time.
- When planning hands forward an authored artifact under review plus non-`none` `EVIDENCE-EXPANSION`, and `INTENT-CLASS` includes both review and research, keep those as separate lane-owned packets rather than collapsing them into one mixed-purpose dispatch. `researcher` owns evidence expansion, contradiction mapping, and source-family coverage; `reviewer` owns artifact judgment, finding classification, and acceptance-surface closure. If `INDEPENDENT-READ-POSSIBLE: yes`, default to parallel `researcher` + `reviewer`. If you keep them serial, name the exact dependency in `PARALLEL-GROUPS` or the runtime blocker that forced the change.
- For that authored-artifact review shape, the immediate `researcher` packet should stay bounded to evidence expansion only: preserve `DECISION-TARGET`, `QUESTION-BOUNDARY`, `SOURCE-FAMILY`, `OUTPUT-SURFACE`, and `DOWNSTREAM-CONSUMER` without absorbing review classification or downstream corrective closure. The immediate `reviewer` packet should stay bounded to judgment only: preserve `REVIEW-TARGET`, `REVIEW-SCOPE`, `PREREQ-STATE`, `EVIDENCE-BASIS`, and `ACCEPTANCE-SURFACE` without absorbing repository-wide evidence gathering or implementation/proof work.
- Lane mix is not staffing completeness. After `researcher`/`reviewer` routing is chosen, size each lane from `WORK-SURFACE-COUNT`, `JUDGMENT-AXIS-COUNT`, `READ-VOLUME`, `CROSS-CHECK-MODE`, and `SHARD-BASIS`. If either lane would still own 3+ independent surfaces or judgment axes, add shards or name the exact dependency, overlap, or context-cost reason for staying narrower.
- `CROSS-CHECK-MODE` values: `lane-separated` means evidence expansion and judgment are independent but not same-surface mirrored; `mirrored-same-surface` means the user-visible conclusion depends on independent challenge over the same declared surface groups. In mirrored mode, freeze one explicit surface map first, then align reviewer coverage to those same declared surface groups rather than assigning one global reviewer a different catch-all surface.
- When mirrored same-surface challenge is required, use sharded reviewer packets with explicit `SHARD-ID`, `SHARD-BOUNDARY`, and `MERGE-OWNER`, or an equivalent child-boundary packet that keeps the same surface map legible. Different-surface fan-out alone is broader coverage, not cross-check.
- When planning marks an authored artifact under review but `EVIDENCE-EXPANSION: none`, prefer `reviewer`-only routing unless another explicit lane-owned surface independently justifies broader staffing. Do not add `researcher` by habit when the supplied artifact and bounded supporting basis already cover the review question.
...
- For workflow-governed discovery, staffing is phase-local, not just overall. If planning records `DISCOVERY-SPLIT-SHAPE`, operationalize that same split at dispatch time or name the exact runtime, overlap, or blocker reason for narrowing it. Do not let one global `researcher` silently absorb workbook extraction, rule interpretation, UI interpretation, and similar derived surfaces when the inherited discovery basis already separated them.
- If the inherited discovery basis is missing but the packet still mixes extraction/inventory with 2+ interpretive surfaces, treat that as a planning defect or repair point before dispatch rather than flattening into one bounded-looking researcher prompt.
...
- Before dispatch construction, compile the worker packet from the frozen phase intent and inherited basis already established upstream (governing workflow phase, planning basis, and any prerequisite handoff/coverage-delta). Do not widen the packet while writing it to absorb adjacent phase work, unresolved prerequisite closure, or lead-owned synthesis merely because those surfaces are nearby. When the inherited basis includes verification continuity, keep that basis row-mapped: downstream review, testing, and validation packets should inherit explicit row state, stale rows, deferred rows, and assumption deltas rather than rediscovering them from broad artifact summaries.
...
- Do not create a new `Agent` with the same concrete worker identity while that worker is still live, standby, working, or lifecycle-pending. Resolve reuse, standby, or shutdown on the existing identity first.
- If 2 or more completed workers are still awaiting an explicit lifecycle decision, treat that as a dispatch backlog defect and resolve the backlog before adding more fan-out.
...
- A turn-ended runtime signal means the worker's current response turn ended, not that the worker is not working. Before replacing a working worker: ask what blocks, request partial results, consider scope reduction, and allow wait time proportional to scope. Replacement is the last resort.
