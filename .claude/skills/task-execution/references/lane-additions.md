...
- Converged lane work sends `subjob-done` after completion carrier proof.
...
- Before any lane direct tool call, pane/final prose candidate, renderable `SendMessage` field, or `Bash` command text, tool parameter, stdout, or stderr construction, reapply the current message-class render basis and `.claude/reference/work-runtime-boundary-law.md` `### Bash Internal Evidence Capture Contract`.
...
- The handoff requires both the retained carrier containing the common completion spine and `MESSAGE-CLASS: subjob-done` sent to `team-lead` through `SendMessage`.
- Required completion handoff uses both retained carrier and `MESSAGE-CLASS: subjob-done`; disk output, pane/final prose, `status`, and `TaskUpdate` remain supporting signals.
...
- Every lane agent applies the common `confirmed-defect` element test from `.claude/reference/review-and-verification-core-law.md` before using defect language; missing expected behavior, observed deviation, causal link, correction-restores-behavior basis, correction owner, or protected-function no-loss basis keeps the item unpromoted or rejected.
...
- Governance audit, doctrine cleanup, or defect-sweep lanes may complete high-recall discovery plus lane-local first-pass `candidate-classified` or `rejected:<basis>` states inside the assigned surface; binding cross-surface filtering, final rejection, promotion, repair-order selection, removal, patch-worthiness, patch-readiness, or patch/no-patch selection requires team-lead/validator full workflow basis, and assigned `Skill(review-verification)` lenses supply bounded evidence only.
