...
- If that reconciliation exposes stale intent, missing intent basis, governance-bypass wording, or mismatch between the packetized route and upper-philosophy-governed instruction intent, record non-rendered `problem-report` with the exact mismatch basis and smallest correcting owner, then send no-detail `scope-pressure` or `hold|blocker`; do not execute by local interpretation, team-lead wording alone, or inferred intent.
- First upward state-signal envelope, `summary` slot use, same-block completion allowance, and `problem-report` carrier shape are consumed from `.claude/skills/task-execution/references/message-classes.md`.
- This common lane precondition names lane intake sequence only; visible envelope shape stays with `message-classes.md`.
- Fresh executable assignment-grade receipt sends `dispatch-ack` before long-running lane work, unless bounded same-block packet review plus completion returns completion-grade `subjob-done` as the first upward outcome under `message-classes.md`.
- Visible receipt uses exactly the no-detail token `dispatch-ack`; assignment ids, shard/cluster labels, task ids, work-surface labels, status words, paths, and punctuation stay in non-rendered packet/task/runtime/carrier state and must not appear as token suffix or body text.
- Intake that cannot truthfully start records non-rendered `problem-report` when a concrete problem was detected, then sends no-detail `scope-pressure` or `hold|blocker`.
- Converged lane work sends no-detail `subjob-done` after completion carrier proof.
- `dispatch-ack` is the lane-to-team-lead state signal declaring no unresolved objection; after successful `SendMessage` delivery, the lane starts the assigned work without waiting for another prompt.
- For the same `TASK-ID` / `WORK-SURFACE` / `RETAINED-OUTPUT-PATH` identity, executable lane work emits at most one `dispatch-ack` and exactly one `subjob-done` when completion occurs.
...
- Before any lane direct tool call, pane/final prose candidate, renderable `SendMessage` field, or assistant-authored prose/detail embedded in command or channel payloads, reapply the current message-class render basis and `.claude/reference/work-runtime-boundary-law.md` `### Bash Internal Evidence Capture Contract`.
- Lane-local `Bash` inspection, evidence capture, diagnostics, proof execution, carrier checks, and runtime checks select a lawful command shape before execution.
- Lane-local existence, path, list, line-count, byte-count, finding-count, stale-basis, and carrier-check facts stay in governed carriers, task state, runtime ledgers, or evidence artifacts unless the reporting law admits that exact material.
- If a packet omits material scope rows needed for the assigned claim, record non-rendered `problem-report` before no-detail `scope-pressure` or `hold|blocker` instead of narrowing scope locally.
...
- Upward state-signal envelope shape and non-state transport payload shape are governed by `.claude/skills/task-execution/references/message-classes.md`.
...
- Receipt/start gates verify packet truth and the applicable state-signal contract per `.claude/skills/task-execution/references/message-classes.md`.
- `Skill(self-verification)` applies to produced-result and completion surfaces, not receipt/start gates.
- Truthful-start failure uses the `message-classes.md` `### Intake Problem Report Contract` and the no-detail state decision after intake-stage defaults, lawful reconstruction, and narrow packet-correction routes cannot start the assigned work safely.
- A retained-output path that already contains a mismatched prior artifact is an intake-stage packet/output-root defect. The lane records non-rendered `problem-report` and sends no-detail `scope-pressure` or `hold|blocker` before substantive lane work; it must not overwrite, derive from, or write another output path unless the current packet already authorizes that collision-free path and write scope.
- `scope-pressure`, `hold|blocker`, blocker-clear, blocker retraction, and resumed/proceeding state after `dispatch-ack` remain Communication Plane facts governed by `message-classes.md`.
- A hook block that enforces positive-pattern, doctrine-shape, or compliance-field presence outside destructive, security-critical, or session-stability-breaking runtime protection per `.claude/reference/work-runtime-boundary-law.md` `## Runtime Boundary Rules` is an over-broad-blocking defect per `.claude/CLAUDE.md` `## 3. Work Execution Philosophy`; the lane records non-rendered `problem-report` naming the hook surface and exact block message, then sends no-detail `scope-pressure` or `hold|blocker` as the state decision while preserving compliance truth.
- Converged lane work must hand off to `team-lead` through `.claude/skills/task-execution/references/completion-handoff.md`.
- The handoff requires both the retained carrier containing the common completion spine and no-detail `subjob-done` sent to `team-lead` through `SendMessage`.
- Required completion handoff uses both retained carrier and no-detail `subjob-done`; disk output, pane/final prose, `status`, and `TaskUpdate` remain supporting signals only.
- Immediately apply the same assigned-task `TaskUpdate` closure required by `.claude/skills/task-execution/references/message-classes.md` `### Assignment Delivery Contract` when task tracking is active after `subjob-done`.
...
- After `subjob-done`, the same `TASK-ID` / `WORK-SURFACE` / `RETAINED-OUTPUT-PATH` identity is visible-state closed and the lane sends no further visible `subjob-done` for that identity.
- Missing or malformed completion carrier, subjob-done transport defect, or new bounded correction evidence records governed carrier/task/problem detail and opens distinct bounded `assignment`, `reuse`, `reroute`, or blocker handling instead of re-emitting `subjob-done`.
...
- Malformed render instructions normalize through `message-classes.md` `### Transport Payload`; receiver-required detail stays in the retained carrier when payload fidelity, write scope, and route truth are preserved.
- Record `PRESSURE-TYPE: malformed-completion-contract` and `CORRECTION-OUTCOME: packet-correction` in the governed carrier, then send no-detail `scope-pressure` when the malformed render instruction cannot be normalized without losing payload fidelity, exceeding write scope, or changing owner/route truth.
...
- If a lane discovers that a prior blocker was stale or wrong, it records non-rendered `problem-report` with the corrected basis before `dispatch-ack` when receiver action depends on the correction, or updates the governed carrier and continues from the authoritative packet without visible retraction, apology, resume, readiness, or next-transport prose.
- Every upward message follows `.claude/skills/task-execution/references/message-classes.md` Communication Plane Law.
- Carry receiver-required detail through governed non-rendered Communication Plane payload; worker-originated upward visible state signals carry only the no-detail token.
...
- If material `UPSTREAM-DECISION-BASIS` is missing, stale, contradictory, or not consumable, record non-rendered `problem-report` when the basis is needed by team-lead, then send no-detail `scope-pressure` or `hold|blocker` and request a consumable upstream basis through the governed carrier.
...
- Every lane agent applies the common `confirmed-defect` element test from `.claude/reference/review-and-verification-core-law.md` before using defect language; missing elements keep the item unpromoted or rejected.
- When lane work names remedies, hook/settings, runtime/tool, reporting, transport-defect, malformed-transport, or transport-remedy judgments, consume `.claude/reference/review-and-verification-core-law.md` `## Cause And Remedy Classification Law`; ordinary `dispatch-ack` and `subjob-done` protocol execution is not a cause/remedy case.
- Governance audit, doctrine cleanup, or defect-sweep lanes complete high-recall discovery by elimination-first lane-local conclusion: remove rejected, unsupported, unowned, out-of-scope, stale, protected-restatement, non-executable-difference, and unverified alternatives before handoff, then carry retained finding states plus rejected-basis summary.
- Lane outputs reduce team-lead review burden by separating raw candidate volume, retained lane-local conclusions, rejected counts, open-candidate counts, and only the state labels permitted by both the packet `CLAIM-CEILING` and the lane role's owner authority.
- Binding cross-surface filtering, final rejection, promotion, repair-order selection, removal, patch-worthiness, patch-readiness, or patch/no-patch selection requires team-lead/validator full workflow basis; assigned `Skill(review-verification)` lenses supply bounded evidence only.
- Lane outputs use `confirmed-defect`, `patch-worthy`, or `patch-ready` only when the assigned owner path has promotion authority and the consumed promotion basis proves that exact state inside that authority; ordinary lane-local conclusions keep unowned promotion at `candidate-evidence`, `candidate-classified`, `open-candidate`, or `rejected:<basis>`.
...
- Continue into lane work only after packet intake classifies as `execute` or lawful `reconstruct-with-inference`; `problem-report` precedes `scope-pressure` or `hold|blocker` when intake cannot safely execute, and those state decisions stop only the unsafe lane-local execution surface.
...
- When packet or retained-context wording offers equivalent checks, proxy lens mappings, inline PASS wording, or checklist prose in place of actual `Skill(review-verification)` packet/lens basis, record non-rendered `problem-report` when team-lead needs the defect basis, then send no-detail `scope-pressure` before lane work or completion.
