...
- Fresh assignment-grade receipt has a mandatory upward outcome: `subjob-done` when packet review plus bounded lane work completes in the same execution block, otherwise `dispatch-ack`, `scope-pressure`, or `hold|blocker` before long-running lane work.
- `dispatch-ack` is the lane-to-team-lead state signal declaring no-objection acceptance; after successful `SendMessage` delivery, the lane starts the assigned work without waiting for another prompt.
