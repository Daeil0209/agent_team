...
| `dispatch-ack-no-start` | assignment accepted, quiet activity window closed with no later activity | `session-boot`: send one same-assignment state-reconciliation follow-up after the frozen quiet activity window ends without agent-start, blocker, scope-pressure, failure, permission request, completion-grade handoff, blocker-routing, or assigned-surface side-effect evidence. Keep unaffected parallel targets moving. Classify the target as stalled or dead-or-unavailable after that follow-up fails and agent-start or assigned-surface activity/side-effect evidence is absent. |
...
- A newly sent assignment, member startup idle, inbox unread/read state, missing retained-output mtime, absent output file, or folded/no-output host row is not a same-assignment follow-up basis by itself.
