...
| `dispatch-pending-no-ack` | assignment sent, no acceptance | `session-boot`: keep the receipt window under monitoring; send one same-assignment state-reconciliation follow-up only after the bounded receipt window or mailbox-consumed-without-channel-response evidence proves missing receipt. Keep unaffected parallel targets moving. Classify the target as dead-or-unavailable for the current assignment after that follow-up fails and assigned-surface activity/side-effect evidence is absent. |
...
| `dispatch-ack-no-start` | assignment accepted, explicit quiet activity window elapsed after valid `dispatch-ack`, and concrete liveness/activity sources still show no later activity | `session-boot`: send one same-assignment state-reconciliation follow-up only after the frozen quiet activity window ends without agent-start, blocker, scope-pressure, failure, permission request, completion-grade handoff, blocker-routing, or assigned-surface side-effect evidence. Keep unaffected parallel targets moving. Classify the target as stalled or dead-or-unavailable after that follow-up fails and agent-start or assigned-surface activity/side-effect evidence is absent. |
...
| `direct-agent-call-incomplete` | legacy or accidental synchronous direct `Agent` call outside team runtime has no returned result | Inspect current host/runtime state, preserve the direct call only as bounded evidence, and reopen `work-planning`, `session-boot`, or `task-execution` redispatch for the configured lane route. Use blocker-routing only after duplicate-send risk and every executable recovery route are exhausted. Completion claims require completion-grade evidence. |
| `direct-agent-result-returned` | legacy or accidental synchronous direct `Agent` result outside team runtime returned | `team-lead` can consume it only as bounded evidence. Load and learn `Skill(self-verification)` for convergence before reporting or redispatch. Team-runtime completion requires team-runtime `subjob-done` transport. |
...
- A newly sent assignment, member startup idle, inbox unread/read state, missing retained-output mtime, absent output file, folded/no-output host row, or after-`dispatch-ack` absent/empty/zero/unchanged artifact observation is not a same-assignment follow-up, recovery-record, idle, no-start, or agent-start-failure basis by itself.
- A bounded follow-up is governed by `message-classes.md` `### Lead Recovery Follow-Up Contract`; it is state-reconciliation control only, not a second assignment, retry, proceed prompt, or instruction to send `subjob-done`.
...
- Ack counts, missing target names, inbox read state, follow-up consideration, replacement consideration, and no-start classification stay internal while Procedure Plane or Communication Plane recovery can continue.
...
- The one bounded follow-up asks only for the missing receipt, missing post-`problem-report` state decision, start evidence, `status`, `scope-pressure`, or `hold|blocker`; receiver-required detail travels through non-rendered task state, runtime ledger, or retained carrier when needed, and renderable fields never include assignment content, packet correction content, `proceed`/`continue` wording, natural-language prompts, or `subjob-done` instruction.
- Resend assignment content only when prior send evidence is absent and duplicate side-effect risk is ruled out; otherwise route correction as distinct bounded `assignment`, `reuse`, or `reroute`.
...
- A failed bounded follow-up opens response/activity re-check, dead-or-unavailable classification, replacement, redistribution, or blocker-routing; it never opens a second follow-up to the same target for the same assignment identity.
...
- Agent records non-rendered `MESSAGE-CLASS: problem-report` when a receiver-consumable channel exists, then sends no-detail `hold|blocker`; renderable state fields are omitted when possible, with empty or single ASCII space used only inside non-omittable fields except the state token.
