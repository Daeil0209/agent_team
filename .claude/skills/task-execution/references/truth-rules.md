...
Dispatch-ack receipt semantics, completion transport semantics, one-state-signal envelope, and task-scoped tool identity requirements live at `.claude/skills/task-execution/references/message-classes.md` `### Receipt Event Contract`, `### Transport Payload`, and `### Shared Task State Contract`.
User-facing dispatch status admission lives at `.claude/reference/reporting-user-reporting-law.md`; admitted dispatch status cites the narrowest proven truth label from the canonical owners above.
