...
- `dispatch-ack` = no-objection assignment acceptance and work-start trigger, not later progress, work quality, completion, or acceptance
...
- user-facing reporting is admitted only by `.claude/reference/reporting-user-reporting-law.md`; any admitted dispatch status cites only the narrowest proven truth label and does not call sent/read/receipt `dispatched`, `running`, `started`, or `complete`
...
- `dispatch-ack` opens lane work and no-start recovery if no later activity evidence appears.
...
- `.claude/reference/reporting-user-reporting-law.md` governs whether a user report exists; narrow proven state governs any admitted report claim.
