...
- Completion Detail Non-Renderable Law
...
- `subjob-done` without qualifier means the no-detail completion state-token transport; `MESSAGE-CLASS: subjob-done` is retained-carrier classification text and is not renderable transport body.
...
## Canonical Channel Registry
The team-runtime data path uses the channel families in the table below.
...
| Direct teammate interaction | teammate UI or peer `SendMessage` | user instruction inside current authority or challenger evidence note for active surface; assistant-authored renderable peer-message fields carry no evidence detail, paths, counts, result text, progress narration, or acceptance language | `.claude/skills/team-session-sequences/references/monitoring-lifecycle-detail.md` | receiver uses governed carrier/evidence; ownership, routing, cleanup, task-control, acceptance, or active-surface changes route to `team-lead` |
...
This category defines the canonical no-detail Communication Plane state-token projection; it is not a user-report exception and never admits adjacent explanatory, status, progress, or self-denial prose.
The only automatic screen state-signal envelope tokens in that projection are exactly `dispatch-ack`, `scope-pressure`, `hold|blocker`, and `subjob-done`.
The visible token is the whole visible token. Do not append assignment ids, shard ids, cluster ids, task ids, file paths, status words, punctuation, or stage labels; forms such as `dispatch-ack CL-2`, `dispatch-ack shard-02`, `subjob-done S1`, or `subjob-done: ready` are malformed Communication Plane leakage.
When the `SendMessage` schema exposes `summary`, `summary` is the canonical visible state-signal slot.
Accepted assignment start uses exactly one state-token envelope: `summary: dispatch-ack`.
Correctable unsafe intake uses exactly one state-token envelope after the required non-rendered `problem-report`: `summary: scope-pressure`.
Blocked intake uses exactly one state-token envelope after the required non-rendered `problem-report`: `summary: hold|blocker`.
Completed subjob handoff uses exactly one state-token envelope: `summary: subjob-done`.
For those four state signals, omit the `message` body when possible; when the schema requires a body after the visible token, use single ASCII space.
For one executable assignment identity, long-running lane work sends one `dispatch-ack` before starting and one `subjob-done` after completion; bounded same-block completion sends the completion-grade `subjob-done` as the first upward outcome and never replays either token.
...
If a host-rendered row or prompt line already displays the allowed state token from `summary`, omit the assistant-authored `message` body unless the schema requires it; if required, use single ASCII space. Pane text and final text are omitted.
...
When the required token exists in `summary` or another single visible slot and the schema requires a renderable body, use single ASCII space.
...
Visible completion variants such as `subjob-done <work-id>`, `subjob-done: <detail>`, or a visible body containing `MESSAGE-CLASS`, `WORK-SURFACE`, `RETAINED-OUTPUT-PATH`, `REVIEW-STATE`, counts, paths, result detail, or status text are malformed raw Communication Plane leakage.
...
For a schema that requires `summary` plus string `message`, place the state token only in `summary` and set `message: " "`; repeating the token in `message`, pane text, or final prose is visibly malformed transport.
...
The visible body/message excludes state tokens, `MESSAGE-CLASS`, field labels, receiver-required detail, and report prose; only the single visible state-signal slot (`summary` or host-rendered state header) may contain `dispatch-ack`, `scope-pressure`, `hold|blocker`, or `subjob-done`.

### Transport Payload
Communication payload carries the detail required by the receiving owner through governed carriers; worker-originated upward state-signal envelopes render at most one no-detail visible token in the state-signal slot, and any required renderable body is a single ASCII space.
...
Task identity, assignment class, carrier pointers, result counts, file paths, evidence summaries, result previews, and completion narrative stay in non-rendered task state, assignment evidence, or retained carriers.
...
### Completion Detail Non-Renderable Law
Completion detail is never visible upward transport.
Lane completion sends exactly the no-detail state token `subjob-done` in the single visible state-signal slot and omits every other renderable field when possible.
If the runtime schema requires a body after `summary: subjob-done`, the body is single ASCII space only.
Completion fields such as `MESSAGE-CLASS`, `TASK-ID`, `WORK-SURFACE`, `OUTPUT-SURFACE`, `RETAINED-OUTPUT-PATH`, `REVIEW-STATE`, `TEST-STATE`, `VERDICT`, `EVIDENCE-BASIS`, `VERIFIED-DATA-FEEDBACK`, `OPEN-SURFACES`, counts, paths, summaries, result details, PASS records, blocker detail, and next-owner recommendations stay only in the retained completion carrier, non-rendered task state, runtime ledger, or evidence artifact.
Lane role files, lane skills, assignment packets, and completion-handoff schemas that list completion fields define retained-carrier content only; they are not pane/final prose templates and are not `SendMessage.summary` or `SendMessage.message` templates.
Team-lead receiving visible completion detail classifies it as malformed raw Communication Plane leakage, not stronger completion evidence.
Malformed visible completion detail may be used only as observation evidence to locate or request the governed carrier when no safer route exists; completion-grade acceptance still requires the retained carrier and corrected no-detail transport or an internally reconciled carrier path.
Correction after malformed visible completion detail uses retained carrier, task state, runtime ledger, non-rendered `problem-report`, or a distinct bounded correction assignment; it does not re-emit visible detail and does not add team-lead progress narration.
...
If no receiver-consumable non-rendered problem channel is available, the lane sends only the no-detail state token and opens team-lead recovery to obtain or reconstruct the missing receiver-required detail from runtime evidence, corrected packet, re-dispatch, or blocker-routing. This fallback does not waive receiver-required problem detail, and the lane does not send visible problem detail.
...
If no receiver-consumable carrier path exists and the problem detail is needed for `team-lead` to act, send only the no-detail state token and leave the detail gap as a team-lead recovery duty through corrected packet, runtime evidence, re-dispatch, or blocker-routing. This fallback does not make private memory a carrier, and visible `problem-report` detail must not travel through `SendMessage`.
...
Task tracking is active only when the frozen route requires host task-list identity and assistant-authored task-row fields can remain identity-only/no-detail.
Host-generated task-row decoration, task ids, or status badges are not assistant-authored reports by themselves.
If assistant-authored task subject, description, assignee, owner, note, result, or other renderable task field would append packet detail, evidence, counts, paths, blocker detail, result text, progress narration, acceptance language, or completion narrative to `dispatch-ack`, `scope-pressure`, `hold|blocker`, or `subjob-done`, task tracking is screen-unsafe for that run; stop creating pre-assignment task rows and carry assignment identity through non-rendered carriers/runtime state.
...
Default render-safe task row shape is a compact operator-facing assignment identity such as `review surface-2`, `test surface-1`, `implement module-3`, or `validate final`; avoid generic subjects such as `work-item` when a stable surface, phase, module, or bounded assignment label is available. `description` is omitted when optional; when required, it uses compact identity-only text and never empty or single ASCII space.
...
When a retained assignment carrier would be authored through a user-rendered file-content preview, it is screen-unsafe for non-reportable packet detail. Prefer a complete assignment-grade `SendMessage` only when its user-rendered fields stay screen-safe; otherwise use non-rendered task/runtime state. Use a retained assignment carrier only when its creation path does not project the carrier body to the user, or when the user explicitly requested that exact carrier content and reporting law admits it.
...
Assistant-authored `TaskUpdate` renderable fields other than the host-supported lifecycle status remain omitted when optional and identity-only/no-detail when required; they must not carry retained-carrier fields, evidence summaries, counts, paths, result previews, acceptance statements, completion narratives, blocker detail, or recovery rationale.
...
A valid `dispatch-ack` moves the target to `ACTIVE`; after that, a follow-up is invalid unless `dispatch-recovery.md` classifies elapsed-window no-start from explicit runtime/liveness/activity evidence. Missing retained output, empty assigned-output surface, zero assigned-output count, no mtime change, or no visible side effect before that classification is not a recovery row.
...
`dispatch-ack` is the no-objection assignment receipt and immediate work-start state token inside the Communication Plane.
...
Receiver-required execution detail travels through the assignment packet, phase-transition packet, shutdown request, non-rendered task state, retained-output carrier, non-rendered `problem-report`, or lead-requested non-rendered status, according to the owning message class.
Visible `dispatch-ack`, `scope-pressure`, `subjob-done`, and `hold|blocker` are state decisions only and never carry receiver-required execution detail.
...
`dispatch-ack` means the lane records no unresolved team-lead correction, replan, authority change, or blocker clearance for the current packet.
...
`dispatch-ack` is edge-triggered for no-objection assignment receipt; after it is visible or ledger-recorded for the current assignment identity, the lane must not send another visible `dispatch-ack` for that identity.
...
A visible state token is edge-triggered and idempotent for the current sender plus non-rendered assignment identity keys: `TASK-ID`, `WORK-SURFACE`, `ACTIVE-SLICE`, and `RETAINED-OUTPUT-PATH`; those keys never appear as visible token suffixes or body text.
...
A valid `dispatch-ack` is successful governed transport, clears receipt, and establishes no-unresolved-objection assignment receipt; later progress, work quality, completion, and acceptance require their own governed evidence.
After valid `dispatch-ack`, missing retained output, empty assigned-output surface, zero assigned-output count, no mtime change, or no visible side effect before closing transport or elapsed-window recovery evidence is expected-not-yet-produced state. Team-lead records monitoring truth internally and does not use those observations as no-start evidence, progress prose basis, same-assignment follow-up basis, proceed prompt basis, status-reconciliation basis, or completion-prompt basis.
...
Renderable team-scoped `Agent` fields are identity/setup fields only: `name` is the stable live member address, `team_name` is the active team address, `description` is a compact render-safe member identity label, and `prompt` carries only the startup screen-safety template.
`Agent.name`, `team_name`, `description`, and `prompt` never carry assignment packet fields, retained-carrier pointers, evidence summaries, counts, paths, route topology, progress, blocker detail, result text, acceptance language, or completion detail.
...
If no assignment packet arrives, the lane waits without assignment work and sends no startup readiness transport.
...
## Upward Message Classes
Every class below is Communication Plane transport. The descriptions name when to use the class; worker-originated visible state signals carry only no-detail tokens, and receiver-required execution truth travels through non-rendered packets, non-rendered task state, retained-output, retained carriers, runtime ledgers, or evidence artifacts.
...
- `dispatch-ack`
  - no-objection receipt and immediate work-start state signal; follows `Receipt Event Contract`
  ...
  - newly discovered blockers after no-objection intake record non-rendered `problem-report` before no-detail `scope-pressure` or `hold|blocker` when team-lead needs the problem basis and a non-rendered channel is available
...
- `subjob-done`
  ...
  - no files-read counts, result counts, per-class totals, excerpts, evidence summaries, operational notes, completion narrative, path-substitution rationale, next-step prose, or retained-output contents in the `SendMessage` render
