...
Load this skill when the Primary Operating Loop classifies a message as Task request, Correction (after self-growth dispatch), or complex Question requiring Standard or Precision tier execution. agents/team-lead.md §RPA-1 §Tier Routing directs loading this skill after completing the Fresh Turn Dispatch Gate for consequential dispatch work. For Standard/Precision-shaped work, this skill is mandatory, not optional guidance. During an SV-only audit that pivots into execution, this skill may load during read-only routing classification to determine tier or runtime fit, but that early load does not satisfy the current-turn `work-planning`/`self-verification` dispatch gate by itself.
...
**Runtime-prep exception:** Once current-turn `work-planning` is observed, `TeamCreate` may prepare runtime before current-turn post-planning `self-verification` completes. Early `task-execution` bootstrap may classify tier or routing fit, but it does not authorize runtime creation by itself. This is runtime preparation only. `Agent`, `TaskCreate`, and assignment-grade `SendMessage` remain blocked until Decision Freeze completes with current-turn `work-planning` plus post-planning `self-verification`.

Before any consequential tool use, apply the reference `Consequential Tool Recovery Contract`: answer routine status-only turns without mutation, treat explicit runtime recovery states as continuation/correction instead of routine status, do not probe hooks, and do not retry a blocked tool until the hook's `Next:` action has been completed.
...
5. Dispatch: Dispatch-initial = initial fan-out dispatch (all immediately unblocked workers). Dispatch-dependent = continuous dependent dispatch (dispatch workers as prerequisites complete). Dispatch-initial and Dispatch-dependent form a single continuous dispatch phase; phase labels mark the transition from initial to dependent dispatch, not separate rounds. Apply Agent Dispatch Discipline. Dispatch all independent work from the plan simultaneously. For dependent phases, sequence only what has actual dependencies. Filter research results — pass only what the plan requires. Continue dispatching dependent-phase workers as prerequisites complete throughout this dispatch phase.
   ...
   - Workload-sizing rule: after lane fit is frozen, size each lane from `WORK-SURFACE-COUNT`, `JUDGMENT-AXIS-COUNT`, `READ-VOLUME`, `CROSS-CHECK-MODE`, and `SHARD-BASIS`. Do not stop at lane presence alone if one worker would still absorb 3+ independent surfaces or judgment axes.
   - Workflow-discovery sizing rule: when the inherited planning basis includes `DISCOVERY-SPLIT-SHAPE`, `DISCOVERY-SURFACE-MAP`, or `DISCOVERY-SERIAL-REASON`, re-check that phase-local basis before the first discovery dispatch. If discovery still mixes extraction/inventory with 2+ interpretive surfaces, do not flatten it into one global `researcher`; use the recorded `scout-then-shards` / `immediate-shards` shape or stop and repair the planning basis.
...
- For concrete patch-set re-dispatch, follow the patch-set re-dispatch rule at `reference.md §Canonical Dispatch Preflight`.
...
| 2 | Scope proportionality: is work proportional to request? Is the dispatch within 2 focused questions, 5 explicitly named file paths, and 1 judgment type? Decompose if exceeded. Exception: `researcher` in `deep` mode, or `reviewer`/`researcher` on a single-judgment cross-surface audit/review packet (for example balance-audit, governance-audit, or systemic cross-file review), may receive broader file scope when that broader read surface is required to support one bounded judgment and the packet remains one phase intent. The question count limit still applies, and this exception does not waive mixed-phase, wrong-owner, proof, implementation, or validation decomposition rules. | Reduce scope or decompose |
...
| 10 | Governance-patch routing needed? Dispatching a developer for governance-sensitive file modifications? Classify the governance-patch tier first (`reference.md § Governance-Patch Packet Tiers`): Standard (single-file, typo/clarification/compliance) uses developer-general + 3 guardrails (TASK-CLASS, CHANGE-BOUNDARY, WRITE-SCOPE); Structural (multi-file, migration, architecture) adds BASELINE-CLASS + EDIT-OPERATION. For meaning migration, CHANGE-BOUNDARY must include source-meaning inventory, destination rationale, and cross-reference list. | Route through Checkpoint C |
...
| 12 | Defect-fix dispatch? If assigning a developer to fix an observed defect, include `TASK-CLASS: defect-fix`, `DEFECT-BASIS`, `REPRO-STATUS`, `REGRESSION-GUARD`, `RETEST-SURFACE`, and `FALLBACK-BASIS`. Practical reproducible defects default to guard-first or simulation-first; impossible or disproportionate guards require explicit fallback proof. | Add defect-fix packet |
| 13 | UI acceptance dispatch explicit? For executable, user-facing UI work, does the acceptance path explicitly state delivery-surface criteria and tool mapping: reviewer on delivery experience + user-readiness, tester on browser-level proof with Playwright CLI, validator on Playwright MCP visual inspection? | Add the missing acceptance-surface and tool requirements |
| 14 | Research dispatch: Dispatching a researcher? Use the Researcher template from `reference.md § Dispatch Packet Templates`. | Apply template |
| 15 | Authored-artifact review with evidence expansion? If planning handed forward `ARTIFACT-UNDER-REVIEW` plus non-`none` `EVIDENCE-EXPANSION` and mixed review+research intent, did dispatch keep `researcher` and `reviewer` as separate lane-owned packets, and if `INDEPENDENT-READ-POSSIBLE: yes`, is any serial choice explicitly justified? | Split or justify serial |
| 16 | Workload-aware staffing complete? If planning handed forward `WORK-SURFACE-COUNT`, `JUDGMENT-AXIS-COUNT`, `READ-VOLUME`, `CROSS-CHECK-MODE`, or `SHARD-BASIS`, did dispatch size the lane count accordingly instead of flattening broad work into one global worker per lane? | Split or justify staffing |
| 17 | SendMessage dispatch: All downward `message-class: assignment` messages must include the base packet plus the target-specific packet. Governance-patch, manifest-sync, and solution-development task-classes are exempt from the User-Perspective Acceptance Gate. See `reference.md § Dispatch Packet Templates` for full field specifications per lane. | Apply template |
...
Decomposition is mandatory for work spanning more than 5 files, requiring both investigation and judgment, or crossing multiple categories. Narrow exception: file-count decomposition may be skipped for a `reviewer` or `researcher` packet whose broader read surface still supports exactly one bounded audit/review judgment, stays within 2 focused questions, and preserves one phase intent without absorbing implementation, proof, validation, or another lane's closure. Decomposition may also be skipped for single-file bounded reads or direct follow-up messages to an active worker on the same surface.
