...
2. Skill Applicability Search: After freezing the request-fit packet, search available skills for workflow, methodology, and domain skills that fit `REQUIRED-DELIVERABLE` and `REQUEST-INTENT`.
   - Scan skill descriptions in the system prompt against the frozen deliverable class. If candidates exist, verify their Activation Criteria for fit.
   - For the overall task: if a governing workflow skill matches (e.g., `dev-workflow`, `doc-auto`), load it before planning or dispatch; its phase structure governs Step 3 Plan. If no match exists, continue with generic tier procedure and record `ACTIVE-WORKFLOW: none` in plan basis.
   ...
   - `task-execution` is the sole owner of governing workflow selection for Standard and Precision tier work. Absence of a governing workflow match is valid; do not force a project workflow onto non-project work.
...
- For researcher/developer dispatch: does this dispatch include `SKILL-RECOMMENDATIONS` when applicable skills were identified during Skill Applicability Search? Dispatch without skill recommendations when applicable skills exist = missed routing opportunity.
...
| 10 | Governance-patch routing needed? Dispatching a developer for governance-sensitive file modifications? Use the Developer (governance-patch) template from `reference.md § Dispatch Packet Templates`. See `reference.md § Governance-Patch Packet Tiers` for tier field specifications. | Route through Checkpoint C |
