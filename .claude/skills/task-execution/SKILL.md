...
- Assignment-grade `SendMessage` uses the assignment-class transport defined by `references/message-classes.md`: no-detail summary token, one visible carrier/task-state pointer line, and complete `MESSAGE-CLASS: assignment` packet fields inside the governed carrier or task state.
- Assignment-grade `SendMessage` summary and body pass the canonical assignment-carrier-pointer envelope in `references/message-classes.md`; hook denial opens same-owner envelope correction and actual `SendMessage` retry.
