...
- **REVIEW-CRITERIA-SOURCE consumption-proof (MANDATORY)**: every codex invocation MUST cause codex to read and learn the full current content of each named REVIEW-CRITERIA-SOURCE file during the invocation before any review claim uses that source. Path acknowledgment, awareness signals, summaries, short quotations, and one-off extracts are access or citation-freshness evidence only; they do not prove criteria-source consumption.
- Codex consumption-proof verification mechanism: codex first response MUST include a `REVIEW-CRITERIA-SOURCE-CONSUMPTION` record for EACH named REVIEW-CRITERIA-SOURCE file with source path, current file line-count or byte-count evidence captured from the invoked source, covered heading/section inventory, first/last section identity, a short verbatim extract for citation freshness, and applied-rule mapping to the review claim.
- Consumption-proof absence = governance-loading failure. Missing full-file coverage evidence, missing first/last section identity, missing applied-rule mapping, or a bare statement such as "fully read" without the required record makes the invocation INVALID and the output INADMISSIBLE.
...
- Invoking-owner verification duty: after receiving codex response, invoking owner MUST verify the current file access evidence, heading/section match, first/last section identity, and citation extract against actual current file content (via own Read or Bash grep); mismatch or missing applied-rule mapping = governance-loading failure → re-invoke or `Skill(governance-modification)` for canonical-path recovery.
