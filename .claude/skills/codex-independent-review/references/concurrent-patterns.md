...
- Governance-loading verification: every codex invocation's first response MUST be evaluated by the invoking owner for Codex Subagent Doctrine awareness signals (Constitutional Reporting Curtain awareness OR §1-§8 philosophy chain awareness OR consumed-surface keyword awareness per Codex `AGENTS.md`). These awareness signals prove operating-governance load plausibility only; they do not prove `REVIEW-CRITERIA-SOURCE` consumption.
...
- REVIEW-CRITERIA-SOURCE default mapping:
  ...
  - Reviewing cross-cutting work: invoking owner names operator-specified canonical criteria source explicitly; absence opens `Skill(team-meeting)` when it can derive a §1-§8-compliant reversible criteria-source path, and routes to operator escalation only when no standing directive, no cited procedure, and genuine novel policy remain.
...
- **REVIEW-CRITERIA-SOURCE consumption-proof (MANDATORY)**: every codex invocation MUST cause codex to read the current sections, trigger blocks, and local resolve/handoff rules in each named REVIEW-CRITERIA-SOURCE file that control the review claim. Whole-file reading is required only for whole-file claims, unknown section boundaries, or structural-contract review. Path acknowledgment, awareness signals, summaries, short quotations, and one-off extracts are access or citation-freshness evidence only; they do not prove criteria-source consumption.
- Codex consumption-proof verification mechanism: codex first response MUST include a `REVIEW-CRITERIA-SOURCE-CONSUMPTION` record for EACH named REVIEW-CRITERIA-SOURCE file with source path, covered heading/section inventory, a short verbatim extract for citation freshness, and applied-rule mapping to the review claim.
- Consumption-proof absence = governance-loading failure. Missing relevant-section coverage evidence, missing applied-rule mapping, or a bare statement such as "fully read" without the required record makes the invocation INVALID and the output INADMISSIBLE.
...
- Invoking-owner verification duty: after receiving codex response, invoking owner MUST verify the current file access evidence, heading/section match, and citation extract against actual current file content (via own Read or Bash grep); mismatch or missing applied-rule mapping = governance-loading failure → re-invoke or `Skill(governance-modification)` for canonical-path recovery.
