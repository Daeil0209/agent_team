...
- usable program -> operator-exhaustive proof of every frozen `SCOPE-BASELINE` feature/surface/control and workflow path, every `CORE-WORKFLOW-CLOSURE` row closed end-to-end on the user surface (create→disposition, edit↔save+reload-persist, input→dependent-recompute/reactivity, UI-element→navigation-destination, entity→related-entity trace), UI/browser tool plan when applicable, plus launch and termination burden in the primary operator environment
