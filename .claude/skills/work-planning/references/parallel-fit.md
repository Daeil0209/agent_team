...
Operational burden is a team-runtime planning input, not a direct-Agent route basis outside team runtime; handle it by cap, shard merge, sub-batching, serial phasing, reuse, or blocker-routing after internal recovery is exhausted without losing configured lane ownership or independent specialist separation.
...
- Binding-surface materialization and quiet collision preflight are Procedure Plane moves, not report reasons.
- Do not announce planning-freeze completion, binding-surface materialization, shard-manifest creation, retained-carrier creation, output-root choice, staging, or collision-preflight rationale.
- Internal rubrics, binding surfaces, shard manifests, retained carriers, and assignment payload content must not be mirrored through assistant-authored prose or visible message fields; materialize full detail through governed carrier paths and verify it with the required read-class or tool-result evidence.
- Before materializing a shared binding surface or retained carrier, or dispatching any assignment/reuse packet that references that carrier or retained-output path, run a quiet collision preflight on the intended path. If the path already exists and reuse is not frozen, choose a collision-free versioned path or same-owner structured edit path from the frozen basis; if reuse is frozen, verify the existing carrier quietly before dispatch.
- A write failure, existing-path fact, missing-directory fact, output-root creation failure, retained-path preflight contradiction, or stale-prior-carrier fact is internal routing evidence only; it consumes `.claude/reference/environment-output-root-filesystem-law.md` `## Output-Root Failure Recovery` or opens quiet path correction, carrier regeneration, structured edit, or blocker-routing, never assistant-authored visible recovery narration.
