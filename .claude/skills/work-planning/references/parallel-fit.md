...
- When the binding surface materializes as an external file or shared retained carrier, the lead stages it under the canonical path defined by `.claude/reference/output-root-and-filesystem-law.md` (default `claude_doc/<work-name>/`).
...
- If the first verification observes absent or empty content while creation evidence exists, the lead waits a bounded propagation window of a few seconds and re-runs the read-class verification.
- If verification still fails after the propagation window, the lead regenerates the carrier from the frozen basis and reruns verification before dispatch.
- If verification still fails after one regeneration cycle, the lead opens `hold|blocker` with unresolved carrier-persistence basis and does not dispatch.
