...
- Hold|blocker-class transport uses the blocker-native fields in `.claude/skills/task-execution/references/message-classes.md`: `BLOCKER-TYPE`, `BLOCKER-BASIS`, and `SAFE-NEXT-STEP`. Keep those fields in the governed payload carrier or retained-output path rather than rendered `SendMessage` body text. Add completion context fields in the payload carrier only when they are known and help team-lead correct, replan, continue independent lanes, or classify a true blocker.
