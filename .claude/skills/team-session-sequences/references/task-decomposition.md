...
1. **Identify atomic steps** - Each dispatch should have a single clear purpose (e.g., "compare this named file group" not "compare, classify, analyze risk, and summarize").
...
Decomposition is mandatory for broad file sets, mixed investigation plus judgment, or cross-category work. Decomposition may be skipped for single-file bounded reads or direct follow-up messages to an active agent on the same surface.
