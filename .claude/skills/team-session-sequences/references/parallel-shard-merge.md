...
- Name one explicit `MERGE-OWNER` before full fan-out whenever the parent task will require a recomposed deliverable, integrated judgment, or final report.
...
- Each shard completion carries at minimum:
  ...
  - downstream placement when the parent artifact will be recomposed
...
- If shard outputs expand the volume beyond the intended artifact class, optimize at merge time.
- Demote or trim locally correct but non-decisive material before acceptance routing.
