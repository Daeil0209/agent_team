...
- Name one explicit `MERGE-OWNER` before full fan-out whenever the parent task will require a recomposed deliverable, integrated judgment, or final report. `MERGE-OWNER` may be the lead or a designated agent, but it must be explicit before shard outputs start accumulating.
...
- Phase separation does not require passive waiting. Starting draft from an incomplete but already useful shard set, then advancing the designated owner through bounded integration follow-up as later shard results arrive, is valid only for interim work while the parent packet, merge ownership, and unresolved gaps stay explicit.
