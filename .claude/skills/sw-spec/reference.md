...
For document-structure/quality see `doc-auto`; for business rules see `biz-sys`. This section owns pipeline software architecture: phase preconditions, gates, input classification, validation contracts.
...
5. **Arrangement** — sequence for burden, flow, control feasibility; venue-type routes to structural profile (profile owned by `biz-sys`)
