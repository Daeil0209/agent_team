...
|---|---|---|---|
| `missing-owner` | No current owner surface governs the recurring behavior. | Assign one owner surface, then add the narrowest rule, skill gate, packet field, or Hook-Last-approved runtime edge needed to close the gap. | Scatter duplicate doctrine across multiple owner files or let the most familiar lane take ownership by habit. |
...
| `non-compliance` | The rule already exists; the failure came from not following it. | Harden adherence through trigger visibility, checklist discipline, packet requirements, self-verification gates, or Hook-Last-approved runtime edge. | Add duplicate rules that make doctrine larger but do not address why the existing rule was missed. |
| `weak-enforcement` | Doctrine exists, but runtime or packet surfaces cannot reliably expose the obligation. | Add or refine packet fields or advisory guidance first; add or refine hook logic only as a Hook-Last-approved runtime edge. | Promote semantic judgment into a hard block before the required fields or low-friction evidence are available. |
