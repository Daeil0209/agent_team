...
- `agents/team-lead.md` §RPA-1. Primary Operating Loop. For IR-1 and §RPA-8. Self-Growth Ownership. For IR-2 contain the atomic classify-dispatch rule and lead-owned adjudication boundary

This skill also contains the Change Sequence and Patch Execution Method used when applying governance changes.
...
- Standard hardening: behavioral rule changes, new constraints, recurrence barriers. All steps required.
- Structural hardening: architecture-level changes, cross-document impact. All steps + governance review.
...
- Treat self-growth as the default self-healing response to a real defect. Once the problem basis is valid, the team should move from explanation into bounded hardening immediately instead of normalizing the miss as commentary.
...
When a defect or justified challenge is recognized, execute this operating algorithm:

1. Trigger — confirm that the problem basis is real enough to govern behavior now.
2. Stabilize — protect the active request's immediate correctness and safety basis before any broader optimization.
3. Classify — distinguish missing owner/rule/capability from non-compliance with an existing one.
4. Dispatch — open self-growth immediately as a sidecar or nearest-safe-boundary patch path. Do not leave the recurrence path untreated once understood.
5. Harden — patch the correct owner surface: doctrine, skill, reference, hook, role boundary, or reusable capability.
6. Re-verify — rerun or otherwise gather fresh evidence on the affected path.
7. Resume in corrected state — continue the active task with the strengthened operating basis, not the pre-correction behavior.

Self-healing is complete only when the recurrence path is either hardened or explicitly assigned with owner, boundary, and closure basis. Explanation alone is not self-healing.

### Trigger discipline

- On recognizing a behavioral defect (self-detected or user-corrected): Classify (missing rule or non-compliance?) → Immediately dispatch a separate agent for correction via `Self-Growth Sequence` as a sidecar — fix first, then continue in the corrected state. Self-growth must not preempt freezing the active user task's request-fit, deliverable shape, or immediate correctness/safety basis; if the defect directly blocks correct or safe handling of the current request, stabilize that basis first, then dispatch the self-growth sidecar at the nearest safe boundary. If write-surface overlap with active production work exists, queue only until the overlap clears — this is a write-safety constraint, not a priority deferral. Classify and Dispatch are atomic and immediate — analysis without dispatch is failure, not action. If an equivalent rule exists, the defect is non-compliance — harden adherence instead of adding duplicates.
...
- Self-growth dispatch may be immediate, but governance-sensitive file modification is still execution-gated. Use the dispatch to classify, route, inspect, and prepare the bounded patch packet; only destructive or security-sensitive execution requires explicit user confirmation.
...
Readiness, promotion, and adaptation handoff stay under `agents/team-lead.md` §Self-Growth Ownership for lead adjudication and this skill's sequence gates for execution. This skill does not create a second owner for those judgments. When a round reaches hardening or promotion evaluation, keep the target capability surface, current readiness label, evidence basis, and follow-on optimization closure explicit before treating the round as closed.

## Sequence Activation Discipline

When the Self-Growth Sequence or Change Sequence is materially active, name the active sequence explicitly in control updates rather than leaving the sequence state implicit. Keep the current phase, step, or blocking condition visible enough that a successor can interpret the next update against the same active sequence.

Do not silently switch between Self-Growth Sequence and Change Sequence while active runtime depends on one of them. If a stage is optional, that optionality must come from the owning rule, not from habit or convenience.

When building a sequencing decision for self-growth work, derive it from the loaded skill procedure rather than from memory. If the governing procedure cannot be pointed to cleanly, keep the work on HOLD until the procedure basis is explicit.
...
5b. `Execution Gate`
- For governance-sensitive modifications (CLAUDE.md § Document Ownership): complete the reviewed change path before Execute. Explicit user approval is required only when the action is destructive or security-sensitive; reviewed non-destructive governance changes proceed autonomously.
...
- When adding, deprecating, retiring, or merging rules, record the lifecycle state transition explicitly (`ACTIVE` -> `DEPRECATED` -> `RETIRED` -> `MERGED` per `CLAUDE.md` `§Governance Lifecycle Labels`). New rules start as `ACTIVE`.
...
- High-traffic governance surfaces (CLAUDE.md § Document Ownership) require session revalidation and baseline anchoring before modification resumes after interruption.
...
- For every moved, compressed, replaced, or removed meaning, keep `SOURCE-MEANING-INVENTORY` and `DESTINATION-OWNER-MAP` explicit enough that another model can reconstruct the patch from repository state alone.
...
- Verification must prove both sides of the patch: the recurrence path is blocked or explicitly assigned, and the existing design remains closed with no unique meaning lost, no dead cross-reference, and no weakened owner boundary.
...
- Route meaning to the owning surface.
  ...
  - file-scoped governance overlay with mandatory `paths:` frontmatter -> files under `.claude/rules/`
...
- Do not place always-on doctrine in `.claude/rules/`. If the content should load unconditionally, route it into `CLAUDE.md` or the correct owner-local surface instead.
...
- Treat the consuming lane as the default skill-up owner.
  - For current developer-exclusive specialist skills, `developer` performs the skill-up execution while `team-lead` governs routing and adoption.
- When multiple specialist skills are active on one blocked surface, the controlling order is: `biz-sys` -> `sw-spec` -> domain specialists (`edu-spec`, `eng-spec`, `math-spec`) -> `enterprise-arch` -> `doc-auto` -> `ui-ux` -> UI implementation specialists (`ui-mockup`, `design-token`). `bench-sim` and `int-op` are support overlays without controlling-chain position; they may evaluate or integrate without overriding earlier controlling contracts.
