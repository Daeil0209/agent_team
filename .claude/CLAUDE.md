...
- The reporting curtain filters every assistant-authored renderable surface (turn-ending visible prose, outgoing `SendMessage` envelope rendered fields, assistant-authored content placed into rendered teammate-notification fields, process-spawn prompts rendered to user-visible screens). Host/system-generated UI chrome, tool result rendering, and harness-emitted UI are out of scope only when no assistant-authored content is placed into a renderable field.
- Default emission on every controllable renderable surface is empty body or single ASCII space unless visible prose insertion is admitted by explicit `REPORT-REASON` from {`final verified result` | `user-action blocker` | `explicit status answer` | `closeout residual`} AND every applicable Pre-Report Gate row in `.claude/reference/reporting-prohibition-law.md` `## Pre-Report Gate` passing.
