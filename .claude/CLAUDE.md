...
- The reporting curtain filters every assistant-authored renderable surface: visible assistant prose before, between, or after tool calls; assistant-authored stdout/stderr intentionally produced by tool commands; turn-ending visible prose; outgoing `SendMessage` envelope rendered fields; assistant-authored content placed into rendered teammate-notification fields; and process-spawn prompts rendered to user-visible screens. Host/system-generated UI chrome, tool result frames, and harness-emitted UI are out of scope only when no assistant-authored content is placed into their rendered fields or intentionally written to their stdout/stderr.
- Default emission on every controllable renderable surface is empty body or single ASCII space. Autonomous user-visible prose is admitted only by explicit `REPORT-REASON: final verified result` AND every applicable Pre-Report Gate row in `.claude/reference/reporting-prohibition-law.md` `## Pre-Report Gate` passing; non-final narrow exceptions in that law are operator-interrupt or true-no-continuation paths, never progress/status/process reports.
- Executable detail lives in: `.claude/reference/reporting-core-law.md` `## Constitutional 4-Invariant Detail`; `.claude/reference/reporting-prohibition-law.md` (Pre-Report Gate, Report Shape, Non-Reportable Content); `.claude/skills/agent-team-lead/references/reporting-prohibition-rules.md` (pre-emission composition discipline); `.claude/hooks/reporting-curtain-envelope-gate.sh` (SendMessage envelope preblock); `.claude/hooks/sv-tracker.sh` (Skill-result suppression); `.claude/hooks/session-start.sh` (curtain self-validation).
...
- Mapped core laws are the mandatory executable-detail surface for the matching top-doctrine intent; their Reference Map subreferences load and consume only when their triggers are active.
