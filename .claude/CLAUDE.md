...
- Every assistant-authored renderable surface, including ordinary assistant message or pane prose before, between, or after tool calls, first applies `.claude/reference/reporting-prohibition-law.md`; admitted wording then applies `.claude/reference/reporting-user-reporting-law.md`.
...
- Terminal-visible stdout or stderr produced by assistant-authored tool commands is a renderable surface; command-output execution consumes `.claude/reference/work-runtime-boundary-law.md` `### Bash Internal Evidence Capture Contract`.
- Host-generated tool rows, tool errors, runtime state, internal state transitions, and recovery decisions are execution evidence only; report admission and non-reportable-content judgment are owned by `.claude/reference/reporting-prohibition-law.md`.
- No-detail Communication Plane state-signal admission, visible envelope shape, and `SendMessage` slot rules are owned by `.claude/reference/reporting-prohibition-law.md` and `.claude/skills/task-execution/references/message-classes.md`.
- Receiver-required problem detail travels through non-rendered Communication Plane carriers; `problem-report` execution is owned by `.claude/skills/task-execution/references/message-classes.md`.
- Work-start, dispatch-start, lane-start, task-start, work-completion, lane-completion, and task-completion stages do not admit user-facing content by stage identity; stage non-reportability is owned by `.claude/reference/reporting-prohibition-law.md`.
...
- Suppression changes only the user-rendered projection; it never stops Procedure Plane or Communication Plane work, removes receiver-required payload, or turns an assignment packet into an empty packet. Required action and receiver-required payload continue through governed tools, packets, tasks, or carriers.
...
- Team-agent dispatch, receipt, reuse, monitoring, and subjob handoff transport use the team-agent runtime; their execution contracts are owned by `Skill(task-execution)` and its references.
- Team-agent receipt and subjob handoff transport are internal orchestration signals; no-detail state tokens, non-rendered problem records, before-work receipt barriers, and malformed-transport recovery are owned by `.claude/skills/task-execution/references/message-classes.md` and `.claude/skills/task-execution/references/dispatch-recovery.md`.
- Completion-grade lane handoff requires governed `subjob-done` transport plus retained-carrier acceptance; carrier and acceptance execution are owned by `.claude/skills/task-execution/references/completion-handoff.md`.
...
- Skill activation requires actual full-body load-and-learn or current same-session loaded-and-learned basis; eligibility, freshness, inspection-only surfaces, and applied-rule mapping are owned by `.claude/reference/work-skill-reference-binding-law.md`.
- Document and reference consumption requires full read-and-learn of the complete active document or complete triggered section plus applied-rule mapping; consumption proof and discovery-only tool treatment are owned by `.claude/reference/work-skill-reference-binding-law.md`.
...
- Review And Verification separates candidate discovery, promotion filtering, confirmed-defect judgment, patch-worthiness, patch-readiness, final rejection, patch/no-patch selection, and reportable issue selection; the finding-state ladder is owned by `.claude/reference/review-and-verification-core-law.md`.
- Review And Verification separates symptom, source, effect, remedy, hook eligibility, stronger-narrower alternative, and counterexample oracle before behavior-affecting judgment; cause/remedy fields and trigger conditions are owned by `.claude/reference/review-and-verification-core-law.md`.
- Normal Communication Plane protocol execution stays under Work Execution; Review And Verification classifies those signals only when judging a defect, malformed transport, remedy, no-patch, hook/runtime/schema, or reporting claim about them.
- Governance defect reports and review packets preserve each finding-state family as distinct states; exact state and count rules are owned by `.claude/reference/review-and-verification-core-law.md`.
...
- Lower duplicate wording is removed unless the mapped core law classifies it as protected local restatement with named owner and direct-consumption or isolation-safety basis.
...
- Reader comprehension is part of execution force; comprehension-friction promotion and repair follow `.claude/reference/modification-core-law.md` `comprehension-as-execution-force` and `.claude/reference/review-and-verification-core-law.md` `## Minimum Executable Information Law`.
